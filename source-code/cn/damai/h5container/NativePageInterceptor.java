    ...
    public static void interceptToNativePage(Activity activity, int i, String str) {
        ...
        if (AndroidInstantRuntime.support(ipChange, "1278027634")) {
            ...
            if (i == 1) {
                ...
                str2 = gr.DISCOVER_CONTENT_DETAIL;
            } else if (i == 2) {
                saveBundle(bundle, str);
                str2 = gr.DISCOVER_THEME;
            } else if (i == 3) {
                Uri parse = Uri.parse(str);
                Set<String> queryParameterNames = parse.getQueryParameterNames();
                if (!f92.d(queryParameterNames)) {
            ...
            if (!TextUtils.isEmpty(str)) {
                ...
                if (!TextUtils.isEmpty(path)) {
                    if (path.contains(PATH_CONTENT_PAGE)) {
                        if (xf2.h(parse.getQueryParameter("contentId"))) {
                            return 1;
                        }
                    } else if (path.contains(PATH_THEME_PAGE)) {
                        if (xf2.h(parse.getQueryParameter("themeId"))) {
                            return 2;
                        }
                    } else if (!path.contains(PAGE_CLUB_PAGE) || !xf2.h(parse.getQueryParameter("artistId"))) {
                        return 0;
                    } else {
                        return 3;
                    }
