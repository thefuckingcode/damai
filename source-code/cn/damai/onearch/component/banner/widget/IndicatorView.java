...
public class IndicatorView extends View implements Indicator {
    ...
    private int dip2px(float f) {
        ...
        if (!AndroidInstantRuntime.support(ipChange, "-310366015")) {
            return (int) (f * getContext().getResources().getDisplayMetrics().density);
        }
        return ((Integer) ipChange.ipc$dispatch("-310366015", new Object[]{this, Float.valueOf(f)})).intValue();
    }
    ...
    private void drawBigCircle(Canvas canvas, float f) {
        ...
        if (interpolatedOffset < 0.99f) {
            ...
            rectF2.set(indicatorStartX - f5, (f - f2) + f7, indicatorStartX + f5, (f2 + f) - f7);
            canvas.drawRoundRect(this.rectF, f5, f5, this.indicatorPaint);
        }
        if (interpolatedOffset > 0.1f) {
            float f8 = this.indicatorRadius;
            RectF rectF3 = this.rectF;
            rectF3.set(indicatorStartX2 - f6, (f - f8) - f7, indicatorStartX2 + f6, f + f8 + f7);
    ...
    private void drawCircle(Canvas canvas, float f) {
        ...
        rectF2.set(f2 + ((f4 - f2) * interpolatedOffset()), f - f5, interpolatedOffset, f + f5);
        this.indicatorPaint.setColor(this.selectedColor);
        RectF rectF3 = this.rectF;
        float f6 = this.indicatorSelectedRadius;
        canvas.drawRoundRect(rectF3, f6, f6, this.indicatorPaint);
    }
    ...
    private void drawCircleRect(Canvas canvas, float f) {
        ...
        float f5 = indicatorStartX + ratioSelectedRadius;
        float interpolatedOffset = interpolatedOffset();
        float max = this.indicatorSpacing + (Math.max(getRatioRadius(), ratioSelectedRadius) * 2.0f);
        int i = this.selectedPage;
        if ((i + 1) % this.pagerCount == 0) {
            float f6 = max * ((float) (-i));
            f2 = f4 + Math.max(f6 * interpolatedOffset * 2.0f, f6);
            f3 = Math.min(f6 * (interpolatedOffset - 0.5f) * 2.0f, 0.0f);
        } else {
            f2 = f4 + Math.max((interpolatedOffset - 0.5f) * max * 2.0f, 0.0f);
            f3 = Math.min(interpolatedOffset * max * 2.0f, max);
        }
        float f7 = f5 + f3;
        RectF rectF2 = this.rectF;
        float f8 = this.indicatorSelectedRadius;
        rectF2.set(f2, f - f8, f7, f + f8);
        this.indicatorPaint.setColor(this.selectedColor);
        RectF rectF3 = this.rectF;
        float f9 = this.indicatorSelectedRadius;
        canvas.drawRoundRect(rectF3, f9, f9, this.indicatorPaint);
    }
    ...
    private void drawDash(Canvas canvas, float f) {
        ...
        if (interpolatedOffset < 0.99f) {
            ...
            rectF3.set(indicatorStartX2, f - f10, (((ratioSelectedRadius * 2.0f) + indicatorStartX2) + f2) - f3, f + f10);
            RectF rectF4 = this.rectF;
            float f11 = this.indicatorSelectedRadius;
            canvas.drawRoundRect(rectF4, f11, f11, this.indicatorPaint);
        }
        if (interpolatedOffset > 0.1f) {
            ...
            rectF5.set((f12 - (ratioSelectedRadius * 2.0f)) - f3, f - f13, f12, f + f13);
            RectF rectF6 = this.rectF;
            float f14 = this.indicatorSelectedRadius;
            canvas.drawRoundRect(rectF6, f14, f14, this.indicatorPaint);
        }
    }
    ...
    private void drawPagerCountCircle(Canvas canvas, float f) {
        ...
        for (int i = 0; i < this.pagerCount; i++) {
            ...
            canvas.drawRoundRect(rectF2, f3, f3, this.indicatorPaint);
        }
    }

    private float getRatioRadius() {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "800966698")) {
            return this.indicatorRadius * this.indicatorRatio;
        }
        return ((Float) ipChange.ipc$dispatch("800966698", new Object[]{this})).floatValue();
    }

    private float getRatioSelectedRadius() {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "-936039217")) {
            return this.indicatorSelectedRadius * this.indicatorSelectedRatio;
        }
        return ((Float) ipChange.ipc$dispatch("-936039217", new Object[]{this})).floatValue();
    }
    ...
    private float indicatorStartX(int i) {
        ...
        return ((float) getPaddingLeft()) + max + (((max * 2.0f) + this.indicatorSpacing) * ((float) i)) + (this.indicatorStyle == 3 ? 0.0f : (max - ratioRadius) / 2.0f);
    }

    private float interpolatedOffset() {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "-1881616101")) {
            return this.interpolator.getInterpolation(this.offset);
        }
        return ((Float) ipChange.ipc$dispatch("-1881616101", new Object[]{this})).floatValue();
    }
    ...
    private int measureHeight(int i) {
        ...
        if (mode == Integer.MIN_VALUE || mode == 0) {
            return (int) ((Math.max(getRatioSelectedRadius(), getRatioRadius()) * 2.0f) + ((float) getPaddingTop()) + ((float) getPaddingBottom()));
        }
        if (mode != 1073741824) {
            return 0;
        }
    ...
    private int measureWidth(int i) {
        ...
        if (mode == Integer.MIN_VALUE || mode == 0) {
            ...
            return (int) ((Math.max(ratioSelectedRadius, ratioRadius) * 2.0f * ((float) i2)) + (((float) (i2 - 1)) * this.indicatorSpacing) + (ratioSelectedRadius - ratioRadius) + ((float) getPaddingLeft()) + ((float) getPaddingRight()));
    ...
    public View getView() {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "-697634992")) {
            return this;
        }
        return (View) ipChange.ipc$dispatch("-697634992", new Object[]{this});
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (this.pagerCount != 0) {
            float height = (((float) getHeight()) / 2.0f) + 0.5f;
    ...
    public IndicatorView setIndicatorRadius(float f) {
        ...
        if (this.indicatorRadius == this.indicatorSelectedRadius) {
            this.indicatorSelectedRadius = (float) dip2px;
        }
        this.indicatorRadius = (float) dip2px;
        return this;
    }
    ...
    public IndicatorView setIndicatorSelectedRadius(float f) {
        ...
        this.indicatorSelectedRadius = (float) dip2px(f);
        return this;
    }
    ...
    public IndicatorView setIndicatorSpacing(float f) {
        ...
        this.indicatorSpacing = (float) dip2px(f);
        return this;
    }
    ...
    public IndicatorView(Context context, @Nullable AttributeSet attributeSet, int i) {
        ...
        this.indicatorRadius = (float) dip2px(3.5f);
        this.indicatorRatio = 1.0f;
        this.indicatorSelectedRadius = (float) dip2px(3.5f);
        this.indicatorSelectedRatio = 1.0f;
        this.indicatorSpacing = (float) dip2px(10.0f);
