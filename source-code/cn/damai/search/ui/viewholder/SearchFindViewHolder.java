...
public class SearchFindViewHolder extends RecyclerView.ViewHolder {
    ...
    private LinearLayout c = ((LinearLayout) this.itemView.findViewById(R$id.layout_pic_item_container));
        ...
        if (AndroidInstantRuntime.support(ipChange, "-2144585700")) {
            ...
            for (int i = 0; i < searchFindWordList.hotWords.size(); i++) {
                ...
                if (searchFindWord != null && !TextUtils.isEmpty(searchFindWord.keyword)) {
                    ...
                    if (!g2 || i >= 4) {
                        view = this.b.inflate(R$layout.layout_hot_text, (ViewGroup) null);
                        TextView textView = (TextView) view.findViewById(R$id.tv_position);
                        textView.setText((i + 1) + "");
                        textView.setTextColor(Color.parseColor(b(i)));
                        ((TextView) view.findViewById(R$id.tv_keyname)).setText(searchFindWord.keyword);
                        e(searchFindWord.getLabelType(), searchFindWord.reason, (TextView) view.findViewById(R$id.tv_label));
                        if (i % 2 == 0) {
                            this.d.addView(view);
                        } else {
                            this.e.addView(view);
                        ...
                        view = this.b.inflate(R$layout.layout_hot_text_pic, (ViewGroup) null);
                        ((TextView) view.findViewById(R$id.tv_keyname)).setText(searchFindWord.keyword);
                        ImageView imageView = (ImageView) view.findViewById(R$id.iv_pic);
                        ImageView imageView2 = (ImageView) view.findViewById(R$id.iv_number);
                        int[] iArr = g;
                        if (i < iArr.length) {
                        ...
                        if (i != 0) {
                            layoutParams.leftMargin = n42.a(this.a, 3.0f);
                        }
                    }
