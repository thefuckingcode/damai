...
public class ScaleCircleNavigator extends View implements NavigatorHelper.OnNavigatorScrollListener, IPagerNavigator {
    ...
    private void init(Context context) {
        ...
        this.mMinRadius = sp2.a(context, 3.0d);
        this.mMaxRadius = sp2.a(context, 3.0d);
        this.mCircleSpacing = sp2.a(context, 4.0d);
    ...
    private void prepareCirclePoints() {
        ...
        if (this.mCircleCount > 0) {
            ...
            for (int i2 = 0; i2 < this.mCircleCount; i2++) {
                this.mCirclePoints.add(new PointF((float) paddingLeft, (float) round));
                paddingLeft += i;
            }
        }
    }
    ...
    public void onDeselected(int i, int i2) {
        ...
        if (AndroidInstantRuntime.support(ipChange, "-1873551896")) {
            ...
            this.mCircleRadiusArray.put(i, Float.valueOf((float) this.mMinRadius));
            invalidate();
        }
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            float floatValue = this.mCircleRadiusArray.get(i, Float.valueOf((float) this.mMinRadius)).floatValue();
            if (i == this.selectIndex) {
                this.mPaint.setColor(this.mSelectedCircleColor);
            } else {
                this.mPaint.setColor(this.mNormalCircleColor);
            }
            canvas.drawCircle(pointF.x, ((float) getHeight()) / 2.0f, floatValue, this.mPaint);
        }
    }
    ...
    public void onEnter(int i, int i2, float f, boolean z) {
        ...
        if (AndroidInstantRuntime.support(ipChange, "1844523266")) {
            ...
            float interpolation = ((float) i3) + (((float) (this.mMaxRadius - i3)) * this.mStartInterpolator.getInterpolation(f));
            this.mCircleRadiusArray.put(i, Float.valueOf(interpolation));
            if (interpolation == ((float) this.mMaxRadius)) {
                this.selectIndex = i;
            }
            invalidate();
        }
    ...
    public void onLeave(int i, int i2, float f, boolean z) {
        ...
        if (AndroidInstantRuntime.support(ipChange, "-664068029")) {
            ...
            float interpolation = ((float) i3) + (((float) (this.mMinRadius - i3)) * this.mStartInterpolator.getInterpolation(f));
            this.mCircleRadiusArray.put(i, Float.valueOf(interpolation));
            if (interpolation == ((float) this.mMaxRadius)) {
                this.selectIndex = i;
            }
    ...
    public void onSelected(int i, int i2) {
        ...
        if (AndroidInstantRuntime.support(ipChange, "-1956314999")) {
            ...
            this.mCircleRadiusArray.put(i, Float.valueOf((float) this.mMaxRadius));
            this.selectIndex = i;
            invalidate();
        }
    }
