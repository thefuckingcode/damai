...
public class CouponStyleLinearLayout extends LinearLayout {
    ...
    private void drawHorCircle() {
        ...
        for (int i = 0; i < this.mCircleNum_H; i++) {
            ...
            float f3 = f + f2 + (this.mRemain_H / 2.0f) + ((f + (2.0f * f2)) * ((float) i));
            this.mCanvas.drawCircle(f3, 0.0f, f2, this.mPaint);
            this.mCanvas.drawCircle(f3, (float) getHeight(), this.mRadius, this.mPaint);
        }
    }
    ...
    private void drawHorEllipse() {
        ...
        for (int i = 0; i < this.mCircleNum_H; i++) {
            float f = this.mGap;
            float f2 = this.mRadius;
            float f3 = f + f2 + (this.mRemain_H / 2.0f) + ((f + (f2 * 2.0f)) * ((float) i));
            ...
            rectF.top = ((float) getHeight()) - this.mRadius;
    ...
    private void drawHorTriangle() {
        ...
        for (int i = 0; i < this.mCircleNum_H; i++) {
            ...
            path.moveTo(f3 - this.mRadius, (float) getHeight());
            path.lineTo(this.mRadius + f3, (float) getHeight());
            path.lineTo(f3, ((float) getHeight()) - this.mRadius);
            path.lineTo(f3 - this.mRadius, (float) getHeight());
            path.close();
            this.mCanvas.drawPath(path, this.mPaint);
        }
    }
    ...
    private void drawVelCircle() {
        ...
        for (int i = 0; i < this.mCircleNum_V; i++) {
            float f = this.mGap;
            float f2 = this.mRadius;
            float f3 = f + f2 + (this.mRemain_V / 2.0f) + ((f + (2.0f * f2)) * ((float) i));
            this.mCanvas.drawCircle(0.0f, f3, f2, this.mPaint);
            this.mCanvas.drawCircle((float) getWidth(), f3, this.mRadius, this.mPaint);
        }
    }
    ...
    private void drawVelEllipse() {
        ...
        for (int i = 0; i < this.mCircleNum_V; i++) {
            float f = this.mGap;
            float f2 = this.mRadius;
            float f3 = f + f2 + (this.mRemain_V / 2.0f) + ((f + (f2 * 2.0f)) * ((float) i));
            RectF rectF = new RectF();
            rectF.left = 0.0f;
            float f4 = this.mRadius;
            rectF.right = f4;
            ...
            rectF.bottom = f3 + f4;
            this.mCanvas.drawOval(rectF, this.mPaint);
            rectF.left = ((float) getWidth()) - this.mRadius;
    ...
    private void drawVelTriangle() {
        ...
        for (int i = 0; i < this.mCircleNum_V; i++) {
            ...
            path.close();
            this.mCanvas.drawPath(path, this.mPaint);
            path.moveTo((float) getWidth(), f3 - this.mRadius);
            path.lineTo((float) getWidth(), this.mRadius + f3);
            path.lineTo(((float) getWidth()) - this.mRadius, f3);
            path.lineTo((float) getWidth(), f3 - this.mRadius);
            path.close();
            this.mCanvas.drawPath(path, this.mPaint);
        }
    }
    ...
    private void measureHorNum(int i) {
        ...
        if (this.mRemain_H == 0.0f) {
            float f = this.mGap;
            this.mRemain_H = (((float) i) - f) % ((this.mRadius * 2.0f) + f);
        }
        float f2 = this.mGap;
        this.mCircleNum_H = (int) ((((float) i) - f2) / ((this.mRadius * 2.0f) + f2));
    }
    ...
    private void measureVelNum(int i) {
        ...
        if (this.mRemain_V == 0.0f) {
            float f = this.mGap;
            this.mRemain_V = (((float) i) - f) % ((this.mRadius * 2.0f) + f);
        }
        float f2 = this.mGap;
        this.mCircleNum_V = (int) ((((float) i) - f2) / ((this.mRadius * 2.0f) + f2));
    }
    ...
    public CouponStyleLinearLayout(Context context, AttributeSet attributeSet, int i) {
        ...
        if (attributeSet != null) {
            ...
            this.mGap = (float) obtainStyledAttributes.getDimensionPixelOffset(R$styleable.CouponStyleLinearLayout_mGap, 5);
            this.mRadius = (float) obtainStyledAttributes.getDimensionPixelOffset(R$styleable.CouponStyleLinearLayout_mRadius, 10);
