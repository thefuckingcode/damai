...
public class ShimmerLayout extends FrameLayout {
    ...
    private Rect calculateBitmapMaskRect() {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "1444698919")) {
            return new Rect(0, 0, calculateMaskWidth(), getHeight());
        }
        return (Rect) ipChange.ipc$dispatch("1444698919", new Object[]{this});
    }

    private int calculateMaskWidth() {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "242106214")) {
            return (int) ((((double) (((float) getWidth()) * this.maskWidth)) / Math.cos(Math.toRadians((double) Math.abs(this.shimmerAngle)))) + (((double) getHeight()) * Math.tan(Math.toRadians((double) Math.abs(this.shimmerAngle)))));
        }
        return ((Integer) ipChange.ipc$dispatch("242106214", new Object[]{this})).intValue();
    }
    ...
    private void drawShimmer(Canvas canvas) {
        ...
        canvas.drawRect((float) rect.left, 0.0f, (float) com.alibaba.wireless.security.aopsdk.replace.android.graphics.Rect.width(rect), (float) com.alibaba.wireless.security.aopsdk.replace.android.graphics.Rect.height(this.maskRect), this.gradientTexturePaint);
        canvas.restore();
    }
    ...
    private Animator getShimmerAnimation() {
        ...
        int width = getWidth();
        if (getWidth() > com.alibaba.wireless.security.aopsdk.replace.android.graphics.Rect.width(this.maskRect)) {
            i = -width;
        } else {
            i = -com.alibaba.wireless.security.aopsdk.replace.android.graphics.Rect.width(this.maskRect);
        }
    ...
    private int reduceColorAlphaValueToZero(int i) {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "-1195940826")) {
            return Color.argb(0, Color.red(i), Color.green(i), Color.blue(i));
        }
        return ((Integer) ipChange.ipc$dispatch("-1195940826", new Object[]{this, Integer.valueOf(i)})).intValue();
    }
