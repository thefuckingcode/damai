...
public class SeatBox {
        ...
        while (it2.hasNext()) {
            ...
            if (f92.d(value) || value.size() < i2) {
                ...
                while (i4 < arrayList2.size()) {
                    ...
                    Path path2 = new Path();
                    float f = (float) i3;
                    path2.addCircle((float) i5, (float) i6, f, Path.Direction.CW);
                    if (i4 == 0) {
                        ...
                        if (i7 == i8 && seatNew2.y == seatNew.y) {
                            ...
                            int i11 = i6 - i9;
                            taoPiaoSeatComparator2 = taoPiaoSeatComparator3;
                            float f2 = (((float) i10) / 2.0f) + ((float) i8);
                            float f3 = (((float) i11) / 2.0f) + ((float) i9);
                            int i12 = i10 * i10;
                            int i13 = i11 * i11;
                            arrayList = arrayList2;
                            hashMap2 = hashMap3;
                            float sqrt = (float) (Math.sqrt((double) (i12 + i13)) / 2.0d);
                            str = str3;
                            i = i4;
                            RectF rectF = new RectF((float) ((int) (f2 - sqrt)), (float) ((int) (f3 - f)), (float) ((int) (f2 + sqrt)), f + f3);
                            Path path4 = new Path();
                            ...
                            if (seatNew2.x == seatNew.x) {
                                ...
                                float f4 = 2.0f * sqrt;
                                try {
                                    d = (Math.acos((double) (((((float) i12) + (f4 * f4)) - ((float) i13)) / (((float) ((i10 * 2) * 2)) * sqrt))) / 3.141592653589793d) * 180.0d;
    ...
    private void createTaoPiaoPathBefore19(Map<Long, List<SeatNew>> map) {
        ...
        for (Map.Entry<Long, List<SeatNew>> entry : map.entrySet()) {
            ...
            if (!f92.d(value) && value.size() >= 2) {
                Collections.sort(value, new Comparator<SeatNew>() {
                    /* class cn.damai.commonbusiness.seatbiz.seat.qilin.bean.SeatBox.AnonymousClass1 */
                    private static transient /* synthetic */ IpChange $ipChange;

                    public int compare(SeatNew seatNew, SeatNew seatNew2) {
                        IpChange ipChange = $ipChange;
                        if (!AndroidInstantRuntime.support(ipChange, "1899547569")) {
                            return seatNew.x - seatNew2.x;
                        }
                        return ((Integer) ipChange.ipc$dispatch("1899547569", new Object[]{this, seatNew, seatNew2})).intValue();
                    }
                });
                Path path = new Path();
                String str = null;
                ...
                while (i < value.size()) {
                    ...
                    if (i == 0) {
                        path.moveTo((float) seatNew.x, (float) seatNew.y);
                    } else {
                        path.lineTo((float) seatNew.x, (float) seatNew.y);
                    }
                    i++;
                    str = str2;
                }
    ...
    public void bindVrData() {
        ...
        } else if (!f92.c(this.seatNewMap) && (map = this.seat3DVrInfoMap) != null && map.size() != 0) {
    ...
    public int getTotalSeatCount() {
        ...
        if (AndroidInstantRuntime.support(ipChange, "1068582311")) {
            ...
            int i2 = 0;
            for (int i3 = 0; i3 < this.seatNewMap.size(); i3++) {
                List<SeatNew> valueAt = this.seatNewMap.valueAt(i3);
                if (valueAt == null) {
                    i = 0;
                } else {
                    i = valueAt.size();
