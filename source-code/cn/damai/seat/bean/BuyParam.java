...
public class BuyParam implements IBuyParam, Serializable {
    ...
    public static String concatBuyParams(@NonNull List<? extends IBuyParam> list) {
        ...
        for (IBuyParam iBuyParam : list) {
            String itemId2 = iBuyParam.getItemId();
            String count2 = iBuyParam.getCount();
            String skuId2 = iBuyParam.getSkuId();
            sb.append(itemId2);
    ...
    public static List<BuyParam> toBuyParamList(long j, ArrayMap<String, ArrayList<SeatGroup>> arrayMap, IPriceManager iPriceManager) {
        ...
        } else if (f92.c(arrayMap) || iPriceManager == null) {
            return null;
        } else {
    ...
    public boolean equals(Object obj) {
        IpChange ipChange = $ipChange;
        if (AndroidInstantRuntime.support(ipChange, "439119253")) {
            return ((Boolean) ipChange.ipc$dispatch("439119253", new Object[]{this, obj})).booleanValue();
        } else if (!(obj instanceof BuyParam) || this.skuId != ((BuyParam) obj).skuId) {
            return false;
        } else {
            return true;
        }
