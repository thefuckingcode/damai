...
public class BigNotePresenter extends BasePresenter<BigNoteModel, BigNoteView, BaseSection> implements BigNoteContract.Presenter<BigNoteModel, BigNoteView, BaseSection> {
    ...
    public class a implements View.OnClickListener {
        ...
        public void onClick(View view) {
            ...
            try {
                if (((BigNoteModel) BigNotePresenter.this.getModel()).getBean().contentInfo.id != null) {
                    bundle.putString("targetId", ((BigNoteModel) BigNotePresenter.this.getModel()).getBean().contentInfo.id);
                }
                bundle.putInt("targetType", lk1.j(((BigNoteModel) BigNotePresenter.this.getModel()).getBean().subType, 1));
    ...
    public BigNotePresenter(BigNoteView bigNoteView, String str, w9 w9Var) {
        super(bigNoteView, str, w9Var);
    }
    ...
    private View addShareView(boolean z) {
        ...
        linearLayout.addView(q92.g(getContext().getActivity(), new a()));
        return linearLayout;
    }
    ...
    public static boolean isTrackInfoValid(TrackInfo trackInfo) {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "612815190")) {
            return trackInfo != null && !TextUtils.isEmpty(trackInfo.trackB) && !TextUtils.isEmpty(trackInfo.trackC);
        }
        return ((Boolean) ipChange.ipc$dispatch("612815190", new Object[]{trackInfo})).booleanValue();
    }
    ...
    public void OnPraiseViewClick(boolean z, String str, int i) {
        ...
        f.put("quanziid", ((BigNoteModel) getModel()).getTrackInfo().getArgsMap().get(za.CNT_CONTENT_ID));
    ...
    public void itemClick(BigNoteContract.View view, NoteBean noteBean, int i) {
        ...
        if (AndroidInstantRuntime.support(ipChange, "1934319234")) {
            ...
            if (isTrackInfoValid(this.mTrackInfo)) {
                ...
                putMap("quanziid", ((BigNoteModel) getModel()).getTrackInfo().getArgsMap().get(za.CNT_CONTENT_ID), hashMap);
                putMap("alg", noteBean.alg, hashMap);
                putMap(za.PUBLISHER_ID, noteBean.baseUserDO.havanaIdStr, hashMap);
    ...
    public void shareClick(ContentShareInfo contentShareInfo, int i) {
        ...
        HashMap<String, String> f = a03.f();
        f.put("quanziid", ((BigNoteModel) getModel()).getTrackInfo().getArgsMap().get(za.CNT_CONTENT_ID));
        f.put("item_id", ((BigNoteModel) getModel()).getBean().contentInfo.id);
        TrackType trackType = TrackType.click;
        String str = ((BigNoteModel) this.mModel).getTrackInfo().trackB;
        userTrack(trackType, null, str, "item_" + i, "share", f, false);
        View addShareView = addShareView(((BigNoteModel) getModel()).getBean().mySelf);
        Bundle bundle = new Bundle();
    ...
    public void init(BigNoteModel bigNoteModel) {
        ...
        BigNoteView bigNoteView = (BigNoteView) getView();
        bigNoteView.setData(bean, bean.pos);
        if (isTrackInfoValid(this.mTrackInfo)) {
            HashMap hashMap = new HashMap();
            if (this.mTrackInfo.getArgsMap() != null) {
                hashMap.putAll(this.mTrackInfo.getArgsMap());
            ...
            TrackType trackType = TrackType.expose;
            View rootView = bigNoteView.getRootView();
