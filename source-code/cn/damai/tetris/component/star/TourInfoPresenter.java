...
public class TourInfoPresenter extends BasePresenter<TourInfoContract.Model, TourInfoView, BaseSection> implements TourInfoContract.Presenter<TourInfoContract.Model, TourInfoView, BaseSection> {
    ...
    public TourInfoPresenter(TourInfoView tourInfoView, String str, w9 w9Var) {
        super(tourInfoView, str, w9Var);
    }
    ...
    private void addCities(List<TourCityItem> list) {
        ...
        if (AndroidInstantRuntime.support(ipChange, "-1381330966")) {
            ...
            LinearLayout linearLayout = (LinearLayout) ((TourInfoView) getView()).getTourCityView().findViewById(R$id.star_tour_info_cities_parent);
            ...
            while (i2 < list.size()) {
                ...
                layoutParams.setMargins(i2 == 0 ? s50.a(this.mContext.getActivity(), 21.0f) : 0, 0, s50.a(this.mContext.getActivity(), 12.0f), 0);
    ...
    private void bindBaseInfo(TourInfoItem tourInfoItem, ViewGroup viewGroup) {
        ...
        ob.a((TextView) viewGroup.findViewById(R$id.kv_star_tour_info_pricevalue), tourInfoItem.getTourPriceLow());
        ob.a((TextView) viewGroup.findViewById(R$id.kv_star_tour_info_name), tourInfoItem.getTourName());
        ob.a((TextView) viewGroup.findViewById(R$id.kv_star_tour_info_desc), tourInfoItem.getTourDesc());
    }
    ...
    private void bindCity(View view, TourCityItem tourCityItem, boolean z, int i) {
        ...
        ob.a((TextView) view.findViewById(R$id.star_tour_info_cityname), tourCityItem.getCityName());
        ob.a((TextView) view.findViewById(R$id.star_tour_info_citydesc), tourCityItem.getVenueName());
        ob.a((TextView) view.findViewById(R$id.star_tour_info_citydate), tourCityItem.getShowTime());
        HashMap hashMap = new HashMap();
    ...
    private void bindTag(View view, String str) {
        ...
        ob.a((TextView) view.findViewById(i), str);
    ...
    private void hideAndStopVideo() {
        ...
        ProxyVideoView videoView = ((TourInfoView) getView()).getVideoView();
    ...
    private void showAndPlayVideo(TourInfoBean tourInfoBean) {
        ...
        String str2 = trackInfo == null ? StarIndexFragment.Default_PAGE : trackInfo.trackB;
        if (trackInfo == null) {
            str = "anchor_tour";
        } else {
            str = trackInfo.trackC;
        }
        this.mVideoView.setListener(new vk(getContext().getActivity(), this.mVideoView, str2, str));
        ((TourInfoView) getView()).getView().addOnAttachStateChangeListener(new b());
        ...
        if (data != null) {
            if (!TextUtils.isEmpty(data.getTourVideoUrl())) {
                ...
                if (data.getTourInfo() != null) {
                    ...
                    bindBaseInfo(tourInfo, ((TourInfoView) getView()).getKVInfoView());
                    addTags(tourInfo.getTourTags(), ((TourInfoView) getView()).getKVInfoView());
                }
            } else {
                hideAndStopVideo();
                if (data.getTourInfo() != null) {
                    ((TourInfoView) getView()).getKVInfoView().setVisibility(8);
                    ((TourInfoView) getView()).getNKVInfoView().setVisibility(0);
                    ((TourInfoView) getView()).getNKVInfoView().removeAllViews();
                    View inflate = LayoutInflater.from(getContext().getActivity()).inflate(R$layout.layout_star_card_tourinfo_projinfo, ((TourInfoView) getView()).getNKVInfoView(), false);
                    ((TourInfoView) getView()).getNKVInfoView().addView(inflate);
                    ...
                    bindBaseInfo(tourInfo2, ((TourInfoView) getView()).getNKVInfoView());
                    addTags(tourInfo2.getTourTags(), ((TourInfoView) getView()).getNKVInfoView());
                }
            }
            if (data.getTourCities() != null && data.getTourCities().size() > 0) {
                addCities(data.getTourCities());
                a aVar = new a(data);
                ((TourInfoView) getView()).getKVInfoView().setOnClickListener(aVar);
                ((TourInfoView) getView()).getNKVInfoView().setOnClickListener(aVar);
            }
        }
    }
}
