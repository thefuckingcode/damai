...
public class WVDevelopTool extends WVApiPlugin {
    ...
    private void updateConfig(WVCallBackContext wVCallBackContext, String str) {
        ...
        try {
            Map<String, String> configs = OrangeConfig.getInstance().getConfigs("windvane_domain");
            if (configs != null) {
                if (configs.size() != 0) {
                    ...
                    TaoLog.i("WVConfig", "receive name=[" + "windvane_domain" + "]; config=[" + jSONObject2.toString() + jl1.ARRAY_END_STR);
                    wVCallBackContext.success();
                    return;
                }
            }
