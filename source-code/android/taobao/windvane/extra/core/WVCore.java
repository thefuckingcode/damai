...
public class WVCore {
    ...
    private boolean checkRemoteIsolateOpen() {
        ...
        return !Arrays.asList(wVCommonConfigData.brandBlacks).contains(Build.getBRAND()) && !Arrays.asList(wVCommonConfigData.modelBlacks).contains(Build.getMODEL()) && !Arrays.asList(wVCommonConfigData.systemBlacks).contains(Build.VERSION.getRELEASE());
    }
    ...
    public boolean checkIsolateIfOpen(Context context) {
        ...
        if (read != null) {
            try {
                ...
                if (!TextUtils.isEmpty(str)) {
                    ...
                    if (split.length == 2) {
                        if (System.currentTimeMillis() - Long.valueOf(split[1]).longValue() > ((long) WVCommonConfig.commonConfig.recoverMultiInterval)) {
                            updateIsolatePolicy(context, true);
                        }
                        z = false;
                        String str2 = TAG;
                        TaoLog.e(str2, "isolate policy: remote=[" + checkRemoteIsolateOpen + "], local=[" + z + jl1.ARRAY_END_STR);
                        if (!checkRemoteIsolateOpen && z) {
                            return true;
                        }
                    }
                }
        ...
        z = true;
        String str22 = TAG;
        TaoLog.e(str22, "isolate policy: remote=[" + checkRemoteIsolateOpen + "], local=[" + z + jl1.ARRAY_END_STR);
        return !checkRemoteIsolateOpen ? false : false;
    }
    ...
    public String getV8SoPath() {
        ...
        if (i == 1) {
            ...
            String str = TAG;
            TaoLog.i(str, "get v8 path by inner lib so, path=[" + sb2 + jl1.ARRAY_END_STR);
            return sb2;
        } else if (i != 2) {
            String ucSoPath = getUcSoPath(UCCore.getExtractDirPathByUrl(GlobalConfig.context, WVUCWebView.UC_CORE_URL));
            String str2 = TAG;
            TaoLog.i(str2, "get v8 path by download so, path=[" + ucSoPath + jl1.ARRAY_END_STR);
            return ucSoPath;
        } else {
            String ucSoPath2 = getUcSoPath(GlobalConfig.getInstance().getUcLibDir());
            String str3 = TAG;
            TaoLog.i(str3, "get v8 path by inner asset so, path=[" + ucSoPath2 + jl1.ARRAY_END_STR);
            return ucSoPath2;
        }
    }
