...
public class WVUrlMatchUtils {
    ...
    private boolean isLoadAppMonitor = false;
    ...
    private int computeGravitySum(ArrayList<String> arrayList) {
        ...
        for (int i2 = 0; i2 < arrayList.size(); i2++) {
            if (arrayList.get(i2).equals(jl1.MUL) || arrayList.get(i2).equals(jl1.PLUS)) {
                i--;
            } else {
    ...
    private void travelTree(UrlParseRes urlParseRes, boolean z, TireNode tireNode, ArrayList<String> arrayList, int i, Map<ArrayList<String>, String> map) {
        ...
        if (urlParseRes != null && tireNode != null) {
            if (tireNode.subField != null && z) {
                if (i5 == urlParseRes.subPaths.length) {
                    ...
                    if (map2 != null && map2.containsKey(jl1.MUL) && tireNode.nodeMap.get(jl1.MUL).group != null && checkSchemeAndPort(urlParseRes, tireNode.nodeMap.get(jl1.MUL))) {
                        arrayList.add(jl1.MUL);
                        map.put(new ArrayList<>(arrayList), tireNode.nodeMap.get(jl1.MUL).group);
                        arrayList.remove(jl1.MUL);
                        return;
                    }
                    return;
            ...
            String lowerCase = (z ? urlParseRes.subPaths[i5] : urlParseRes.subHosts[i5]).toLowerCase();
            if (tireNode.nodeMap.containsKey(lowerCase)) {
                int i6 = i5 + 1;
                arrayList.add(lowerCase);
                if (z || i6 != urlParseRes.subHosts.length) {
                    z2 = z;
                    i4 = i6;
                } else {
                    z2 = true;
                ...
                travelTree(urlParseRes, z2, tireNode.nodeMap.get(lowerCase), arrayList, i4, map);
                int i7 = i4 - 1;
                if (i7 < 0) {
                    i5 = urlParseRes.subHosts.length - 1;
                    z2 = false;
                } else {
                    i5 = i7;
                }
            ...
            if (tireNode.nodeMap.containsKey(jl1.PLUS)) {
                if (!z2) {
                    i3 = 0;
                    z4 = true;
                } else {
                    z4 = z2;
                    i3 = urlParseRes.subPaths.length;
                }
                arrayList.add(jl1.PLUS);
                travelTree(urlParseRes, z4, tireNode.nodeMap.get(jl1.PLUS), arrayList, i3, map);
                arrayList.remove(jl1.PLUS);
                if (i3 == 0) {
                    z4 = false;
                }
                z3 = z4;
            ...
            if (!tireNode.nodeMap.containsKey(jl1.MUL)) {
                return;
            }
            if (!z3 || i5 != 0 || !TextUtils.equals(lowerCase, "/")) {
                if (!z3) {
                    z3 = true;
                    i2 = 0;
                } else {
                    i2 = urlParseRes.subPaths.length;
                }
                arrayList.add(jl1.MUL);
                travelTree(urlParseRes, z3, tireNode.nodeMap.get(jl1.MUL), arrayList, i2, map);
                arrayList.remove(jl1.MUL);
                return;
            }
            arrayList.add(jl1.MUL);
            travelTree(urlParseRes, z3, tireNode.nodeMap.get(jl1.MUL), arrayList, i5, map);
            arrayList.remove(jl1.MUL);
        }
    }
        ...
        if (!(jSONArray == null || jSONArray.length() == 0)) {
            for (int i = 0; i < jSONArray.length(); i++) {
                try {
                    ...
                    if (jl1.MUL.equals(string)) {
                        return true;
                    }
                    int indexOf = string.indexOf(46);
                    if (indexOf > 0 && string.substring(0, indexOf).equals(str2)) {
                        int i2 = indexOf + 1;
                        if (jl1.MUL.equals(string.substring(i2)) || str3.equals(string.substring(i2))) {
                            return true;
                        }
                    }
    ...
    public boolean checkSchemeAndPort(UrlParseRes urlParseRes, TireNode tireNode) {
        boolean z;
        boolean z2;
        if (urlParseRes == null || tireNode == null) {
            return false;
    ...
    public String getUrlPermissionInfo(String str, String str2) {
        ...
        if (map == null || map.isEmpty()) {
            ...
            return null;
        } else if (TextUtils.equals(str2, "open") && (str.startsWith("javascript:") || str.startsWith("about:"))) {
    ...
    public boolean isBrowserOpen(String str) {
        ...
        return !PermissionChecker.PERMISSION_ALLOW.equalsIgnoreCase(urlPermissionInfo) && !"allowAll".equalsIgnoreCase(urlPermissionInfo) && !"forbidden".equalsIgnoreCase(urlPermissionInfo);
    }
    ...
    public String searchUrl(String str) {
        ...
        while (true) {
            ...
            if (i2 < strArr.length) {
                ...
                if (tireNode3 == null) {
                    if (!tireNode2.nodeMap.containsKey(jl1.MUL)) {
                        if (!tireNode2.nodeMap.containsKey(jl1.PLUS)) {
                            break;
                        }
                        tireNode2 = tireNode2.nodeMap.get(jl1.PLUS);
                        i2 = parseUrl.subHosts.length;
                    } else {
                        tireNode2 = tireNode2.nodeMap.get(jl1.MUL);
                        i2 = parseUrl.subHosts.length;
                    }
                } else {
                    i2++;
        ...
        if (!tireNode2.nodeMap.containsKey("/") && tireNode2.nodeMap.containsKey(jl1.MUL)) {
            tireNode2 = tireNode2.nodeMap.get(jl1.MUL);
        }
        int i3 = 0;
        while (true) {
        ...
        if (tireNode2.group == null && tireNode2.nodeMap.containsKey(jl1.MUL)) {
            tireNode2 = tireNode2.nodeMap.get(jl1.MUL);
        }
