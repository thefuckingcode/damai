...
public class e8 extends ClassLoaderAdapter {
    private Class<? extends WXComponent> a(String str, WXSDKInstance wXSDKInstance) {
        ...
        if (!(wXSDKInstance == null || (context = wXSDKInstance.getContext()) == null || TextUtils.isEmpty(str))) {
            ...
            if (!(queryIntentServices == null || queryIntentServices.isEmpty() || (serviceInfo = queryIntentServices.get(0).serviceInfo) == null || serviceInfo.name == null)) {
                try {
                    return ((IExternalComponentGetter) e8.class.getClassLoader().loadClass(serviceInfo.name).newInstance()).getExternalComponentClass(str, wXSDKInstance);
                } catch (Throwable th) {
                    th.printStackTrace();
                }
            }
    ...
    private Class<? extends WXModule> b(String str, Context context) {
        ...
        if (context != null && !TextUtils.isEmpty(str)) {
            ...
            if (!(queryIntentServices == null || queryIntentServices.isEmpty() || (serviceInfo = queryIntentServices.get(0).serviceInfo) == null || serviceInfo.name == null)) {
                try {
                    return ((IExternalModuleGetter) e8.class.getClassLoader().loadClass(serviceInfo.name).newInstance()).getExternalModuleClass(str, context);
                } catch (Throwable th) {
                    th.printStackTrace();
                }
            }
