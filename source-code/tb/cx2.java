    ...
    public static void c(WXComponent wXComponent) {
        ...
        if (a && (C = WXSDKManager.v().C()) != null && C.size() != 0) {
            try {
                ...
                for (IWXAnalyzer iWXAnalyzer : C) {
                    iWXAnalyzer.transfer(kx2.WXAPM_CONFIG_GROUP, wXComponent.getInstanceId(), "wxinteraction", jSONObject);
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        if (a) {
            ...
            if (C != null && C.size() != 0 && (wXSDKInstance = WXSDKManager.v().i().get(str)) != null) {
                try {
                    ...
                    for (IWXAnalyzer iWXAnalyzer : C) {
                        iWXAnalyzer.transfer(kx2.WXAPM_CONFIG_GROUP, wXSDKInstance.getInstanceId(), str2, jSONObject);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
