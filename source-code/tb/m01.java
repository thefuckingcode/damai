...
public class m01 {
    ...
    public static void b(RecyclerView recyclerView) {
        ...
        if (AndroidInstantRuntime.support(ipChange, "1201600049")) {
            ...
            g91.b("OrangeSwitch", "feed scroll opt " + a + " 1=开启");
            if (a != 1) {
                onScrollListener = new InstantImgLoader();
            } else if (Build.VERSION.SDK_INT >= 19) {
                onScrollListener = new LazyImageLoader();
                recyclerView.addOnScrollListener(onScrollListener);
            } else {
                onScrollListener = new InstantImgLoader();
            }
            recyclerView.setTag(R$id.id_img_loader_tag, onScrollListener);
        }
    }
