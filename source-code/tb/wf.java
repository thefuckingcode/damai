...
public class wf {
            ...
            while (it.hasNext()) {
                ...
                if (!f92.d(sections)) {
                    for (BaseSection baseSection : sections) {
                        if (TextUtils.equals(baseSection.getComponentId(), wj2.DM_PROJECT_LIST_BELOW_FILTER_PANEL_CID) && baseSection.getItem() != null) {
                            try {
                                ...
                                categoryItemListInfo.currentCitySize = xf2.e(categoryItemListBean.currentCity);
                                categoryItemListInfo.nearByCitySize = xf2.e(categoryItemListBean.nearByCity);
    ...
    public static boolean b(BaseResponse baseResponse) {
        ...
        if (baseResponse != null && f92.g(baseResponse.layers)) {
            Iterator<BaseLayer> it = baseResponse.layers.iterator();
            while (it.hasNext()) {
                List<BaseSection> sections = it.next().getSections();
                if (f92.g(sections)) {
                    for (BaseSection baseSection : sections) {
                        if (TextUtils.equals(wj2.DM_PROJECT_FILTER_PANEL_CID, baseSection.getComponentId())) {
                            return true;
                        }
                    }
                    continue;
