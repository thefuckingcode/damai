...
public class cc implements IBizStat {
    private static String[] a = {c.c, c.d, "v3", "v4", "v5"};
    ...
    private void a(BizStatData bizStatData) {
        if (!(TextUtils.isEmpty(bizStatData.module) || TextUtils.isEmpty(bizStatData.monitorPoint))) {
            if (TextUtils.isEmpty(bizStatData.module) || !bizStatData.monitorPoint.equals(b.get(bizStatData.module))) {
                ...
                create.addDimension(if1.DIMEN_POD_VERSION);
                create.addDimension("errorCode");
                create.addDimension(if1.DIMEN_MESSAGE);
            ...
            create3.setValue(if1.DIMEN_BIZ, bizStatData.bizId);
            create3.setValue("etag", bizStatData.etag);
            create3.setValue(if1.DIMEN_POD_VERSION, bizStatData.podver);
            create3.setValue(if1.DIMEN_VERSION, bizStatData.appSnapshotVersion);
            create3.setValue("digest", bizStatData.digest);
            create3.setValue("errorCode", String.valueOf(bizStatData.code));
            create3.setValue(if1.DIMEN_MESSAGE, bizStatData.message);
    ...
    public void commitDownload(BizStatData bizStatData) {
        if (c) {
            int i = 0;
            if (bizStatData == null) {
                o22.e("BizStat", "commitUse statData null", new Object[0]);
                return;
            }
            DimensionValueSet create = DimensionValueSet.create();
            create.setValue(if1.DIMEN_BIZ, bizStatData.bizId);
            create.setValue("etag", bizStatData.etag);
            create.setValue(if1.DIMEN_POD_VERSION, bizStatData.podver);
            create.setValue(if1.DIMEN_VERSION, bizStatData.appSnapshotVersion);
            create.setValue("digest", bizStatData.digest);
            create.setValue("errorCode", String.valueOf(bizStatData.code));
            create.setValue(if1.DIMEN_MESSAGE, bizStatData.message);
            ...
            AppMonitor.Stat.commit(if1.MODULE_NAME, "slide_download", create, create2);
            a(bizStatData);
        }
    }
    ...
    public void commitUse(BizStatData bizStatData) {
        if (c) {
            ...
            AppMonitor.Stat.commit(if1.MODULE_NAME, "slide_use", create, create2);
            a(bizStatData);
        }
    }
}
