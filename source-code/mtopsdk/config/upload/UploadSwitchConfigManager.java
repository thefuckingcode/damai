...
public class UploadSwitchConfigManager {
    ...
    /* compiled from: Taobao */
    public static class UploadSwitchConfigManagerInstanceHolder {
    ...
    public void parseRemoteSegmentSizeMapConfig() {
        ...
        if (!StringUtils.isBlank(config)) {
            try {
                Map map = (Map) JSON.parseObject(config, new TypeReference<Map<String, Integer>>() {
                    /* class mtopsdk.config.upload.UploadSwitchConfigManager.AnonymousClass1 */
                }, new Feature[0]);
