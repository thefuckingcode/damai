...
public class MaEngineAPI {
    ...
    public DecodeResult[] doProcess(byte[] bArr, Camera camera, Rect rect, Camera.Size size, int i, boolean z, int i2, float f) {
        ...
        if (bArr == null) {
            ...
            if (size == null || i < 0) {
                size2 = camera.getParameters().getPreviewSize();
            } else {
                size2 = size;
            }
            if (size2 == null) {
            ...
            return doProcess(bArr, rect, new Point(size2.width, size2.height), i, z, i2, f);
        }
    }
    ...
    public DecodeResult[] doProcess(byte[] bArr, Rect rect, Point point, int i, int i2, boolean z, int i3, float f) {
        ...
        DecodeResult[] decodeResultArr2 = null;
        if (bArr == null) {
            MaLogger.d(TAG, "doProcess mData == null");
            MaBuryRecord.reportUnusualScanCase(1, "mData is null");
            return null;
        ...
        if (rect2 == null) {
            ...
            if (z) {
                decodeResultArr = MaAnalyzeAPI.decodeBinarizedData(bArr, com.alibaba.wireless.security.aopsdk.replace.android.graphics.Point.getx(point), com.alibaba.wireless.security.aopsdk.replace.android.graphics.Point.gety(point), rect2, i3, DecodeType.getCodeTypes(decodeType, this.b));
            } else {
                if (this.d == null) {
                    this.d = decodeType;
                    MaLogger.d(TAG, "doProcess: recognizeType=" + this.a + ", typeSet=" + this.b);
                }
                decodeResultArr = MaAnalyzeAPI.decodeScanMode(bArr, com.alibaba.wireless.security.aopsdk.replace.android.graphics.Point.getx(point), com.alibaba.wireless.security.aopsdk.replace.android.graphics.Point.gety(point), i, rect2, DecodeType.getCode(decodeType, this.b), f);
            }
            ...
            decodeResultArr2 = decodeResultArr;
        }
