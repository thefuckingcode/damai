...
public class PatchManager {
    ...
    private void d(Patch patch) {
        ...
        Log.d("PatchManager", "loadPatch(patch=" + patch + jl1.BRACKET_END_STR);
        for (String str : patch.getPatchNames()) {
            if (this.h.containsKey(jl1.MUL)) {
    ...
    public Patch addPatch(File file) {
        Log.i("PatchManager", "addPatch(file=" + file + jl1.BRACKET_END_STR);
        if (!file.getName().endsWith(".jar")) {
            return null;
        }
        initAndfixManager();
        try {
            ...
            throw new AndFixException("Failed to addPath(File=" + file + jl1.BRACKET_END_STR, e2);
        }
    }

    public void cleanPatches(boolean z) {
        Log.w("PatchManager", "cleanPatches(force=" + z + jl1.BRACKET_END_STR);
        initAndfixManager();
    ...
    public void loadPatch(String str, ClassLoader classLoader) {
        ...
        for (Patch patch : this.g) {
            if (patch.getPatchNames().contains(str)) {
                Log.d("PatchManager", "loadPatch().fix(patchName=" + str + ", patch=" + patch.getFile() + ", classLoader=" + classLoader + jl1.BRACKET_END_STR);
                a(patch, classLoader, patch.getClasses(str), str);
            }
        }
    }
    ...
    public void init(String str, boolean z, boolean z2) {
        ...
        if (!this.f.exists() && !this.f.mkdirs()) {
            ...
            Log.d("PatchManager", "PatchManager.init(ver=" + string + jl1.BRACKET_END_STR);
    ...
    public void addPatch(String str, boolean z) throws IOException {
        Patch addPatch;
        Log.d("PatchManager", "addPatch(path=" + str + ", immediately=" + z + jl1.BRACKET_END_STR);
    ...
    public void loadPatch() {
        this.h.put(jl1.MUL, this.d.getClassLoader());
        for (Patch patch : this.g) {
            for (String str : patch.getPatchNames()) {
                List<String> classes = patch.getClasses(str);
                Log.d("PatchManager", "loadPatch().fix(patchName=" + str + ", patch=" + patch.getFile() + jl1.BRACKET_END_STR);
