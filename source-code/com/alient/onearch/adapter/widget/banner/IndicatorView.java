...
public class IndicatorView extends View implements Indicator {
    ...
    private void drawLineIndicator(Canvas canvas) {
        ...
        int i3 = i % i2;
        if (i3 < i2 - 1) {
            float f = ((float) (i3 + 1)) * (width / ((float) i2));
            if (this.selectedLineRectF == null) {
                this.selectedLineRectF = new RectF();
            }
            RectF rectF2 = this.selectedLineRectF;
            RectF rectF3 = this.lineBgRectF;
            rectF2.set(0.0f, rectF3.top, f, rectF3.bottom);
            if (this.lineIndicatorCurrPath == null) {
                this.lineIndicatorCurrPath = new Path();
            }
            this.lineIndicatorCurrPath.reset();
            Path path2 = this.lineIndicatorCurrPath;
            RectF rectF4 = this.selectedLineRectF;
            int i4 = this.lineIndicatorRadius;
            path2.addRoundRect(rectF4, (float) i4, (float) i4, Path.Direction.CW);
    ...
    private int measureHeight(int i) {
        ...
        if (mode == Integer.MIN_VALUE || mode == 0) {
            size = (int) ((Math.max(getRatioSelectedRadius(), getRatioRadius()) * 2.0f) + ((float) getPaddingTop()) + ((float) getPaddingBottom()));
    ...
    private int measureWidth(int i) {
        ...
        if (mode == Integer.MIN_VALUE || mode == 0) {
            ...
            size = (int) ((Math.max(ratioSelectedRadius, ratioRadius) * 2.0f * ((float) i3)) + (((float) (i3 - 1)) * this.indicatorSpacing) + (ratioSelectedRadius - ratioRadius) + ((float) getPaddingLeft()) + ((float) getPaddingRight()));
        } else if (mode != 1073741824) {
            size = 0;
        }
    ...
    private void updateLineIndicatorPath() {
        ...
        float f = i > measuredHeight ? (float) measuredHeight : (float) i;
        float f2 = (float) measuredHeight;
        this.lineBgRectF = new RectF(0.0f, (f2 - f) / 2.0f, (float) measuredWidth, (f2 + f) / 2.0f);
        Path path2 = new Path();
        this.lineIndicatorBgPath = path2;
    ...
    public IndicatorView(Context context, @Nullable AttributeSet attributeSet, int i) {
        ...
        setIndicatorSpacing((float) obtainStyledAttributes.getDimensionPixelSize(R.styleable.IndicatorView_indicatorSpacing, 10));
