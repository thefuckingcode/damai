...
public class TextTextureGenerator {
    ...
    private void createTextParam() {
        ...
        float f2 = ((float) (this.TEXT_FONTSIZE - this.TEXT_FONTSIZE_TRUE)) / 2.0f;
        this.start_x = f2;
        float f3 = 7.3242188f;
        try {
        ...
        this.base_line = ((((float) this.TEXT_FONTSIZE_TRUE) - (f3 + f)) / 2.0f) + f2 + 0.5f;
    }
    ...
    public byte[] getTextPixelBuffer(int i, int i2) {
        ...
        try {
            ...
            float f2 = measureText - ((float) this.TEXT_FONTSIZE_TRUE);
            Paint.Align align = Paint.Align.CENTER;
            if (textAlign == align || f2 < 4.0f) {
                canvas.drawText(cArr, 0, 1, this.start_x, f, this.text_paint);
            } else {
                this.text_paint.setTextAlign(align);
                this.text_paint.setTextSize(((float) this.TEXT_FONTSIZE_TRUE) - f2);
                canvas.drawText(cArr, 0, 1, (((float) this.TEXT_FONTSIZE_TRUE) - f2) / 2.0f, f, this.text_paint);
