        ...
        if (!this.inited.get() && WVAppParamsManager.getInstance().isParamsSet() && this.inited.compareAndSet(false, true)) {
            ...
            try {
                ...
                if (!TextUtils.isEmpty(params.appKey)) {
                    ...
                    throw new AndroidRuntimeException("appVersion cannot empty, par ams is set = [" + WVAppParamsManager.getInstance().isParamsSet() + jl1.ARRAY_END_STR);
                }
                throw new AndroidRuntimeException("appKey cannot empty, params is set = [" + WVAppParamsManager.getInstance().isParamsSet() + jl1.ARRAY_END_STR);
            } catch (Throwable th) {
