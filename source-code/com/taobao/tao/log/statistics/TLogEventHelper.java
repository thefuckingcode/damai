...
public class TLogEventHelper {
    ...
    static synchronized void uploadDiagnose(String str, Map<String, String> map) {
        ...
        synchronized (TLogEventHelper.class) {
            try {
                if (!listenMap.isEmpty() && map != null) {
                    if (!map.isEmpty()) {
                        String str3 = "";
                        TLogUploadDiagnoseListener tLogUploadDiagnoseListener = null;
                        if (map.containsKey(TLogEventConst.PARAM_TASK_ID)) {
                            str3 = map.get(TLogEventConst.PARAM_TASK_ID);
                            if (!TextUtils.isEmpty(str3) && listenMap.containsKey(str3)) {
                                tLogUploadDiagnoseListener = listenMap.get(str3);
                            }
                        }
                        ...
                        switch (c) {
                            ...
                            case 14:
                                ...
                                break;
                            case 15:
                                str2 = map.get(TLogEventConst.PARAM_UPLOAD_STAGE);
                                break;
                            default:
                                return;
                        }
                        if (tLogUploadDiagnoseListener != null) {
                            tLogUploadDiagnoseListener.onEvent(str3, str2, str, map);
                        }
                        return;
                    }
                }
