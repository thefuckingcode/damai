...
public class TLogInitializer {
    ...
    public TLogInitializer builder(Context context2, LogLevel logLevel2, String str, String str2, String str3, String str4) {
        ...
        if (this.storeLogOnInternal) {
            this.logDir = context2.getDir("tlog_v9", 0);
        } else {
            File file = null;
            try {
                file = context2.getExternalFilesDir("tlog_v9");
            } catch (Exception e) {
                e.printStackTrace();
            }
            if (file == null) {
                file = context2.getDir("tlog_v9", 0);
            }
            this.logDir = file;
        }
        return this;
    ...
    public String getLogCacheDir(Context context2) {
        return context2.getFilesDir().getAbsolutePath() + File.separator + TLOG_DIR + this.tlogFileVersion;
    }
    ...
    public TLogInitializer init() {
        ...
        try {
            ...
            if (defaultSharedPreferences.contains(TLogConstant.TLOG_CONFIG_LOG_FILE_SIZE)) {
                long j = defaultSharedPreferences.getLong(TLogConstant.TLOG_CONFIG_LOG_FILE_SIZE, 52428800);
                if (j >= 0) {
            ...
            if (defaultSharedPreferences.contains(TLogConstant.TLOG_BUFFER_SIZE)) {
                long j2 = defaultSharedPreferences.getLong(TLogConstant.TLOG_BUFFER_SIZE, 600);
                this.tlogBuffersize = j2;
                if (j2 < 300 || j2 > 3000) {
                    this.tlogBuffersize = 600;
                }
            }
    ...
    public TLogInitializer setTLogStatistics(ITLogStatistics iTLogStatistics2) {
        this.iTLogStatistics = iTLogStatistics2;
        return this;
    }
    ...
    public TLogInitializer setTaoBaoMessageSender(MessageSender messageSender2) {
        this.messageSender = messageSender2;
        if (messageSender2 != null) {
            MessageInfo messageInfo = new MessageInfo();
            messageInfo.context = this.context;
