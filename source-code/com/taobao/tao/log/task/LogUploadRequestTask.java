        ...
        try {
            ...
            if (bool == null || bool.booleanValue() || Boolean.valueOf(TLogUtils.checkNetworkIsWifi(TLogInitializer.getInstance().getContext())).booleanValue()) {
                ...
                if (findFile == null || findFile.size() <= 0) {
                    ...
                    TLogEventHelper.uploadFailEvent(uploadFileType, uploadReason, uploadStage, errorCode.getValue(), "", str);
                    TLogInitializer.getInstance().gettLogMonitor().stageError(TLogStage.MSG_HANDLE, this.TAG, "files == null || files.size() <= 0");
                    LogUploadReplyTask.executeFailure(commandInfo, str, null, "1", errorCode.getValue(), "Can't find the logfie", null);
                    return null;
                }
            ...
            ErrorCode errorCode2 = ErrorCode.UPLOAD_NOT_WIFI;
            TLogEventHelper.uploadFailEvent(uploadFileType, uploadReason, uploadStage2, errorCode2.getValue(), "NotWifi", str);
            LogUploadReplyTask.executeFailure(commandInfo, str, null, "1", errorCode2.getValue(), "NotWifi", null);
            return this;
        } catch (Exception e) {
