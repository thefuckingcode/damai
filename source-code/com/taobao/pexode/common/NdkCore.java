...
public class NdkCore {
    ...
    static {
        try {
            ...
            kg0.f(Pexode.TAG, "system load lib%s.so success", "pexcore");
        } catch (UnsatisfiedLinkError e) {
            kg0.c(Pexode.TAG, "system load lib%s.so error=%s", "pexcore", e);
        }
    }
    ...
    public static boolean a(String str) {
        if (b == null) {
            try {
                ...
                kg0.c(Pexode.TAG, "NdkCore.isCpuAbiSupported error=%s", e);
            }
            if (b == null) {
                if (Build.VERSION.SDK_INT >= 21) {
    ...
    public static void c(Context context) {
        if (!a) {
            ...
            a = z;
            kg0.f(Pexode.TAG, "retry load lib%s.so result=%b", "pexcore", Boolean.valueOf(z));
        }
    }
