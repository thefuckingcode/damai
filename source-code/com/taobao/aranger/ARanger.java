...
public class ARanger {
    ...
    public static void connect(ComponentName componentName) throws IPCException {
        ...
        xg.b(IPCUtils.queryContentAuthorityFromProvider(componentName)).b();
    }
    ...
    private static <T> T createProxy(ComponentName componentName, String str, Class<T> cls, String str2, Pair<Class<?>, Object>... pairArr) throws IPCException {
        ...
        ServiceWrapper serviceInterfaceClass = ServiceWrapper.obtain().setTimeStamp(cm2.a()).setServiceInterfaceClass(cls);
        if (TextUtils.isEmpty(str)) {
            str = serviceName.value();
        }
        ServiceWrapper type = serviceInterfaceClass.setServiceName(str).setType(!TextUtils.isEmpty(str2) ? 1 : 0);
        ParameterWrapper[] c = a.c(pairArr);
        if (TextUtils.isEmpty(str2)) {
            methodWrapper = MethodWrapper.obtain();
        } else {
            methodWrapper = MethodWrapper.obtain().setMethodName(str2);
        }
        Call remoteProviderUri = Call.obtain().setVoid(true).setIsSameApp(sApplication.getPackageName().equals(componentName.getPackageName())).setServiceWrapper(type).setMethodWrapper(methodWrapper).setParameterWrappers(c).setRemoteProviderUri(queryContentAuthorityFromProvider);
        xg.b(queryContentAuthorityFromProvider).f(remoteProviderUri);
            ...
            synchronized (ARanger.class) {
                if (sApplication == null) {
                    ...
                    try {
                        ...
                        jz0.c(TAG, "[getContext][currentActivityThread]", e, new Object[0]);
                    }
                    if (sApplication == null) {
                        try {
                            ...
                            jz0.c(TAG, "[getContext][invoke]", e2, new Object[0]);
                        }
                    }
