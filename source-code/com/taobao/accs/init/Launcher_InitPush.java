...
public class Launcher_InitPush implements Serializable {
    ...
    static {
        ...
        if (brand.equalsIgnoreCase("huawei") || brand.equalsIgnoreCase(AgooConstants.MESSAGE_SYSTEM_SOURCE_HONOR) || "tianyi".equalsIgnoreCase(brand) || "liantong".equalsIgnoreCase(brand)) {
            z = true;
        }
        isHMS = z;
    }
    ...
    public static void monitorPush(final Context context) {
        ...
        if (sharedPreferences.getInt(Constants.KEY_TOKEN_REPORT_STATUS, 0) > 0) {
            w6.b().commitStat(new ManuMonitor(manuMonitor2.sdk, true));
        }
        sharedPreferences.edit().putInt(Constants.KEY_TOKEN_REPORT_STATUS, 1).apply();
        ThreadPoolExecutorFactory.getSendScheduledExecutor().schedule(new Runnable() {
            /* class com.taobao.accs.init.Launcher_InitPush.AnonymousClass1 */
