...
public class Launcher_InitAgooLifecycle implements Serializable {
            ...
            if (!hasConnected) {
                ...
                if (coldLaunchTimeInMill > 0) {
                    ...
                    if (elapsedRealtime > 10000) {
                        AppMonitorAdapter.commitCount("accs", BaseMonitor.COUNT_COLD_LAUNCH_TIME, "", (double) elapsedRealtime);
                    } else {
                        ThreadPoolExecutorFactory.getScheduledExecutor().schedule(new Runnable() {
                            /* class com.taobao.accs.init.Launcher_InitAgooLifecycle.AnonymousClass2 */
    ...
    private void registerConnReceiver(final Context context) {
        ThreadPoolExecutorFactory.execute(new Runnable() {
            /* class com.taobao.accs.init.Launcher_InitAgooLifecycle.AnonymousClass1 */

            public void run() {
                AnonymousClass1 r0 = new BroadcastReceiver() {
                    /* class com.taobao.accs.init.Launcher_InitAgooLifecycle.AnonymousClass1.AnonymousClass1 */
