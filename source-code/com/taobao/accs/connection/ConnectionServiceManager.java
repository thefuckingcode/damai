...
public class ConnectionServiceManager {
    ...
    public void coldLaunch() {
        try {
            ...
            if (isAllWeather()) {
                ...
                ALog.preTag = jl1.ARRAY_START_STR + (SystemClock.elapsedRealtime() / 10000) + "] " + ALog.oriTag;
            }
    ...
    public boolean isCurProcessAllow2Connect() {
        try {
            if (!isAllWeather() || ((AllWeatherConnectionService) this.connService).isCurProcessAllow2Connect()) {
                return true;
            }
            return false;
        } catch (Exception e) {
    ...
    public synchronized boolean isEnabled(Context context) {
        if (this.enabled == null) {
            this.enabled = Boolean.valueOf(!TextUtils.isEmpty(this.serviceType) && !this.serviceType.equals("0"));
        }
    ...
    private ConnectionServiceManager() {
        ...
        this.processStateListener = new ProcessStateListener() {
            /* class com.taobao.accs.connection.ConnectionServiceManager.AnonymousClass1 */
