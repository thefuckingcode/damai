...
public class RetryMonitorDbHelper extends SQLiteOpenHelper {
    ...
    RetryMonitorDbHelper(Context context) {
        super(context, DATABASE_NAME, (SQLiteDatabase.CursorFactory) null, 1);
    }

    public synchronized void delete(long j) {
        try {
            long delete = (long) getWritableDatabase().delete(TABLE_RETRY_MONITOR_INFO, "id = ?", new String[]{String.valueOf(j)});
            if (LogUtils.DEBUG) {
                LogUtils.d(TAG, "delete: id = " + j + ", deleteRows = " + delete);
            }
        } catch (Throwable th) {
            LogUtils.d(TAG, "delete by id exception.", th);
        }
        return;
    }
    ...
    public synchronized void delete(long j, int i) {
        ...
        try {
            long delete = (long) writableDatabase.delete(TABLE_RETRY_MONITOR_INFO, "expire_time > ? or retry_times >= ?", new String[]{String.valueOf(j), String.valueOf(i)});
            if (LogUtils.DEBUG) {
                LogUtils.d(TAG, "delete: deletedRows = " + delete + ", expireTime = " + j + ", maxRetryTimes = " + i);
            }
        } catch (Throwable th) {
            LogUtils.d(TAG, "delete by expire time exception.", th);
        }
    }
    ...
    public synchronized void delete(String str, int i) {
        SQLiteDatabase writableDatabase = getWritableDatabase();
        try {
            long delete = (long) writableDatabase.delete(TABLE_RETRY_MONITOR_INFO, "date != ? or retry_times >= ?", new String[]{str, String.valueOf(i)});
            if (LogUtils.DEBUG) {
                LogUtils.d(TAG, "delete: deletedRows = " + delete + ", date = " + str + ", maxRetryTimes = " + i);
            }
        } catch (Throwable th) {
            LogUtils.d(TAG, "delete by date exception.", th);
        }
    }
    ...
    public synchronized List<RetryMonitorInfo> query(long j) {
        ...
        query = getReadableDatabase().query(TABLE_RETRY_MONITOR_INFO, null, "expire_time <= ?", new String[]{String.valueOf(j)}, null, null, null, null);
        while (query != null) {
            try {
                if (!query.moveToNext()) {
                    break;
    ...
    public synchronized List<RetryMonitorInfo> query(String str) {
        ...
        Cursor query;
        arrayList = new ArrayList();
        query = getReadableDatabase().query(TABLE_RETRY_MONITOR_INFO, null, "date = ?", new String[]{str}, null, null, null, null);
        while (query != null) {
            try {
                if (!query.moveToNext()) {
                    break;
    ...
    public synchronized void update(long j, int i) {
        ...
        try {
            ...
            LogUtils.e(TAG, "update error: id = " + j, th);
        }
        if (LogUtils.DEBUG) {
