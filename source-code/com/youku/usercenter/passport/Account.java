...
public class Account {
    ...
    static Account load(Context context) {
    ...
    private void notifySTokenUpdated() {
        new HYTask(null) {
            /* class com.youku.usercenter.passport.Account.AnonymousClass1 */
    ...
    /* access modifiers changed from: package-private */
    public boolean isLastAccountValid() {
        try {
            Account loadLast = loadLast(PassportManager.getInstance().getConfig().mContext);
            long timestamp = PassportManager.getInstance().getTimestamp();
            if (loadLast == null || timestamp >= loadLast.mExpireTime) {
                return false;
            }
            return true;
        } catch (Throwable th) {
    ...
    public boolean isLogin() {
        return !TextUtils.isEmpty(this.mPToken) || !TextUtils.isEmpty(this.mYktk);
    }
    ...
    public void updateExtraCookie(JSONObject jSONObject) {
        ...
        if (!TextUtils.equals(jSONObject3, str)) {
