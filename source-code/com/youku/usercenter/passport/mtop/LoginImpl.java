...
public class LoginImpl extends BroadcastReceiver implements IRemoteLoginAdapter {
    ...
    public void login(final onLoginListener onloginlistener, boolean z) {
        if (!PassportManager.getInstance().isInit()) {
            ...
            PassportManager.getInstance().uccTrustLogin(!TextUtils.isEmpty(config.mSessionSite) ? config.mSessionSite : "youku", hashMap, new ICallback<AbsResult>() {
                /* class com.youku.usercenter.passport.mtop.LoginImpl.AnonymousClass1 */
        ...
        if (!TextUtils.isEmpty(string)) {
            try {
                ...
                if (!TextUtils.isEmpty(string2)) {
                    j = new Date(string2).getTime();
                }
                PassportManager.getInstance().handleCookieError(parseInt, j);
