...
public class DrawHandlerNew extends Handler implements IDrawHandler {
    ...
    private void prepare(final Runnable runnable) {
        if (this.drawTask == null) {
            this.drawTask = createDrawTask(this.mDanmakuView.isDanmakuDrawingCacheEnabled(), this.timer, this.mDanmakuView.getContext(), this.mDanmakuView.getWidth(), this.mDanmakuView.getHeight(), this.mDanmakuView.isHardwareAccelerated(), new IDrawTask.TaskListener() {
                /* class com.youku.danmaku.engine.controller.DrawHandlerNew.AnonymousClass3 */
                ...
                public void onDanmakuAdd(BaseDanmaku baseDanmaku) {
                    if (!baseDanmaku.isTimeOut()) {
                        ...
                        } else if (j > 0 && j <= DrawHandlerNew.this.mContext.mDanmakuFactory.mMaxDanmakuDuration) {
    ...
    private void updateInNewThread() {
        if (this.mThread == null) {
            AnonymousClass2 r0 = new UpdateThread("DFM Update") {
                /* class com.youku.danmaku.engine.controller.DrawHandlerNew.AnonymousClass2 */
                        ...
                        if (DrawHandlerNew.this.mFrameUpdateRate - (SystemClock.uptimeMillis() - uptimeMillis) > 1) {
                            ...
                            if (syncTimer < 0) {
                                ...
                                if (!DrawHandlerNew.this.mDanmakusVisible) {
                                    DrawHandlerNew.this.waitRendering(10000000);
    ...
    /* JADX WARNING: Removed duplicated region for block: B:103:0x02da  */
    ...
    /* JADX WARNING: Removed duplicated region for block: B:109:0x0306  */
    /* JADX WARNING: Removed duplicated region for block: B:123:0x03ac  */
    /* JADX WARNING: Removed duplicated region for block: B:134:0x043f  */
    ...
    public void handleMessage(Message message) {
        ...
        switch (i2) {
            case 1:
                ...
                if (Log.isDebug()) {
                    Log.d("DrawHandlerNew: START: startTime=" + Log.showTime(l.longValue()));
                }
                if (l != null) {
                    ...
                    if (i2 == 4) {
                        ...
                        if (Log.isDebug()) {
                            Log.d("DrawHandlerNew: SEEK_POS: position=" + Log.showTime(valueOf.longValue()) + ", pausedPosition=timer=" + Log.showTime(this.timer.currMillisecond));
                        }
                    }
                    removeMessages(7);
                    this.quitFlag = false;
                    if (this.mReady) {
                        ...
                        if (Log.isDebug()) {
                            Log.d("DrawHandlerNew: RESUME: timer=" + Log.showTime(this.timer.currMillisecond) + ", pausedPosition=" + Log.showTime(this.pausedPosition));
                            return;
                        }
                        return;
                    } else if (Log.isDebug()) {
                        Log.d("DrawHandlerNew: RESUME: is resumed");
                        return;
                    } else {
                        return;
                    }
            ...
            case 5:
                ...
                if (Log.isDebug()) {
                    Log.d("DrawHandlerNew: PREPARE");
                }
                prepare(new Runnable() {
                    /* class com.youku.danmaku.engine.controller.DrawHandlerNew.AnonymousClass1 */

                    public void run() {
                        DrawHandlerNew.this.mReady = true;
                        if (DrawHandlerNew.this.mCallback != null) {
                            DrawHandlerNew.this.mCallback.prepared();
                        }
                    }
                });
                return;
            ...
            case 7:
                ...
                if (Log.isDebug()) {
                    Log.d("DrawHandlerNew: PAUSE: remove UPDATE, timer=" + Log.showTime(this.timer.currMillisecond));
                }
                this.mIsResume = false;
                removeMessages(3);
                removeMessages(2);
            ...
            case 8:
                ...
                if (Log.isDebug()) {
                    Log.d("DrawHandlerNew: SHOW_DANMAKUS: timer=" + getCurrentTime());
                }
                if (this.drawTask != null) {
                    if (l2 == null) {
                        if (Log.isDebug()) {
                            Log.d("DrawHandlerNew: SHOW_DANMAKUS: start=false");
                        }
                        this.timer.update(getCurrentTime());
                        this.drawTask.requestClear();
                    } else {
                        if (Log.isDebug()) {
                            Log.d("DrawHandlerNew: SHOW_DANMAKUS: start=" + l2);
                        }
                        this.drawTask.start();
                        ...
                        if (!z) {
                            if (Log.isDebug()) {
                                Log.d("DrawHandlerNew: SHOW_DANMAKUS: resume=false");
                                return;
                            }
                            return;
                        }
            ...
            case 9:
                ...
                if (Log.isDebug()) {
                    Log.d("DrawHandlerNew: HIDE_DANMAKUS: quitDrawTask=" + bool + ", timer=" + this.timer.currMillisecond);
                }
        ...
        syncTimerIfNeeded();
        this.pausedPosition = this.timer.currMillisecond;
        loge("YKDanmaku.render", " QUIT or PAUSE,what=" + i2 + ",pausedPosition=" + this.pausedPosition, "other");
        if (Log.isDebug()) {
            Log.d("DrawHandlerNew: QUIT or PAUSE,what=" + i2 + ",pausedPosition=" + this.pausedPosition);
        }
        if (this.mUpdateInSeparateThread) {
            notifyRendering();
            quitUpdateThread();
