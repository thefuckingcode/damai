...
public class SpannedCacheStuffer extends SimpleTextCacheStuffer {
    private void staticLayoutDraw(BaseDanmaku baseDanmaku, Canvas canvas, float f, float f2, TextPaint textPaint) {
        ...
        if (z3 || staticLayout == null) {
            ...
            if (charSequence != null) {
                if (z2) {
                    staticLayout = new StaticLayout(charSequence, textPaint, (int) Math.ceil((double) StaticLayout.getDesiredWidth(baseDanmaku.text, textPaint)), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, true);
    ...
    public void measure(BaseDanmaku baseDanmaku, TextPaint textPaint, boolean z, AndroidDisplayer.DisplayConfig displayConfig) {
        if (baseDanmaku.text instanceof Spanned) {
            CharSequence charSequence = baseDanmaku.text;
            StaticLayout staticLayout = new StaticLayout(charSequence, textPaint, (int) Math.ceil((double) StaticLayout.getDesiredWidth(charSequence, textPaint)), Layout.Alignment.ALIGN_NORMAL, 1.0f, 0.0f, true);
            baseDanmaku.paintWidth = (float) staticLayout.getWidth();
            baseDanmaku.paintHeight = (float) staticLayout.getHeight();
            baseDanmaku.obj = new SoftReference(staticLayout);
            return;
