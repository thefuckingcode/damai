...
public class AndroidDisplayer extends AbsDisplayer<Canvas, Typeface> {
    ...
    public int draw(BaseDanmaku baseDanmaku) {
        ...
        if (baseExtraStyle != null) {
            top += (float) baseExtraStyle.getTopPadding();
            z2 = baseDanmaku.mExtraStyle.isAniImage();
        } else {
            z2 = false;
        }
        if (!z2) {
            z4 = this.sStuffer.drawCache(baseDanmaku, this.canvas, left, top, paint);
        }
        if (!z4) {
            if (paint != null) {
    ...
    public void setDensities(float f, int i, float f2) {
        ...
        displayConfig.mBorderRadius = (((float) i) / 160.0f) * 6.0f;
    }
    ...
    public void drawDanmaku(BaseDanmaku baseDanmaku, Canvas canvas2, float f, float f2, boolean z) {
        BaseCacheStuffer baseCacheStuffer = this.sStuffer;
        if (baseCacheStuffer != null) {
            baseCacheStuffer.drawDanmaku(baseDanmaku, canvas2, f, f2, z, this.mDisplayConfig);
        }
    }
    ...
    public static class DisplayConfig {
        ...
        public DisplayConfig() {
            ...
            paint.setStrokeWidth((float) this.UNDERLINE_HEIGHT);
        ...
        public synchronized TextPaint getPaint(BaseDanmaku baseDanmaku, boolean z) {
            ...
            if (this.HAS_SHADOW) {
                float f = this.SHADOW_RADIUS;
                if (f > 0.0f) {
                    int i = baseDanmaku.textShadowColor;
                    if (i != 0) {
                        textPaint.setShadowLayer(f, 0.0f, 0.0f, i);
