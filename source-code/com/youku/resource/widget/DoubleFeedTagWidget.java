...
public class DoubleFeedTagWidget extends View implements ICssSetter2 {
    ...
    private void drawArrow(Canvas canvas) {
        Drawable drawable2 = arrowDrawable;
        ...
        drawable2.setBounds(i + i2, (i3 - i4) / 2, i2 + iArr[0] + i4, ((measuredHeight - i5) / 2) + i5);
    ...
    private void drawOneLineTitle(Canvas canvas, Paint paint, float f) {
        if (calculateTitleLine(-1, true)) {
            canvas.drawText(this.mTitle, (float) textXOffset, f, paint);
            return;
        }
        TextLine[] textLineArr = this.mLines;
        if (textLineArr[0].size > 0) {
            canvas.drawText(textLineArr[0].text, 0, textLineArr[0].size, (float) textXOffset, f, paint);
        }
    }
    ...
    public void setLeftIcon(String str, int i, final int i2, final int i3) {
        tp1.o().s(str).Q(new IPhenixListener<ug2>() {
            /* class com.youku.resource.widget.DoubleFeedTagWidget.AnonymousClass1 */

            public boolean onHappen(ug2 ug2) {
                if (ug2.f() == null || ug2.i()) {
                    return true;
                }
                DoubleFeedTagWidget.this.drawable = Utils.zoomDrawable(ug2.f(), DoubleFeedTagWidget.this.getResources(), i2, i3);
