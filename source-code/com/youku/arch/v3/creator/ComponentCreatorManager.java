...
public final class ComponentCreatorManager {
    ...
    public final IComponent<ComponentValue> create(@NotNull Config<Node> config) {
        ...
        while (true) {
            ...
            T next = it.next();
            int typeStart = next.getTypeStart();
            int typeEnd = next.getTypeEnd();
            int type = config.getType();
            if (typeStart <= type && type <= typeEnd) {
                ComponentCreator componentCreator = this.componentCreators.get(next);
                if (componentCreator != null) {
                    iComponent = componentCreator.create(config);
                }
                if (iComponent != null) {
                    break;
                }
    ...
    public final boolean isSupport(int i) {
        ...
        for (T t : this.componentCreators.keySet()) {
            if (i >= t.getTypeStart() && i <= t.getTypeEnd()) {
                return true;
            }
        }
        return false;
    ...
    public final void register(@NotNull TypeRange typeRange, @NotNull ComponentCreator componentCreator) {
        ...
        k21.i(componentCreator, "componentCreator");
