...
public class DisplayUtils {
    ...
    public static int dp2px(int i) {
        ...
        return (int) ((((float) i) * getDisplayMetrics().density) + (i > 0 ? 0.5f : -0.5f));
    }
    ...
    public static String getBitmapDensityStr() {
        ...
        if (bitmapDensity == 120) {
            return EX_DENSITY_LOW;
        }
        if (bitmapDensity == 160) {
            return EX_DENSITY_MEDIUM;
        }
        if (bitmapDensity == 240) {
            return EX_DENSITY_HIGH;
        }
        if (bitmapDensity != 320) {
            return bitmapDensity != 480 ? "" : EX_DENSITY_XXHIGH;
        }
    ...
    public static int getDimensionPixelSize(@DimenRes int i) {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "-2005336241")) {
            return AppInfoProviderProxy.getAppContext().getResources().getDimensionPixelSize(i);
        }
        return ((Integer) ipChange.ipc$dispatch("-2005336241", new Object[]{Integer.valueOf(i)})).intValue();
    }
    ...
    public static void initStatusBarHeight(Context context) {
        ...
        try {
            ...
            sSystemStatusBarHeight = (float) context.getResources().getDimensionPixelSize(Integer.parseInt(cls.getField("status_bar_height").get(cls.newInstance()).toString()));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public static boolean isPortrait() {
        IpChange ipChange = $ipChange;
        if (AndroidInstantRuntime.support(ipChange, "1786945227")) {
            return ((Boolean) ipChange.ipc$dispatch("1786945227", new Object[0])).booleanValue();
        }
        if (getConfiguration().orientation == 1 || (getConfiguration().orientation == 0 && getHeightPixels() > getWidthPixels())) {
            return true;
        }
        return false;
    }
