...
public final class NodeParser {
    ...
    private final Node parse(JSONObject jSONObject) {
        ...
        if (!AndroidInstantRuntime.support(ipChange, "-1642190478")) {
            return parse(null, jSONObject);
        }
        return (Node) ipChange.ipc$dispatch("-1642190478", new Object[]{this, jSONObject});
    }
    ...
    private final Node parseNode(String str) {
        ...
        try {
            ...
            Log.v(TAG, k21.r("FastJsonParser cost time AA ", Long.valueOf(System.currentTimeMillis() - currentTimeMillis)));
            k21.h(node, "{\n            val node =…           node\n        }");
            Log.v(TAG, k21.r("FastJsonParser cost time BB ", Long.valueOf(System.currentTimeMillis() - currentTimeMillis)));
            return node;
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
    ...
    public final Node parse(@Nullable Node node, @Nullable JSONObject jSONObject) {
        ...
        node2.setId((long) jSONObject.getIntValue("id"));
        ...
        if (jSONObject2 != null) {
            StyleFactory styleFactory = StyleFactory.INSTANCE;
            if (node != null) {
                style = node.getStyle();
            }
            node2.setStyle(styleFactory.create(jSONObject2, style));
        }
    ...
    private final Node parse(String str) {
        ...
        try {
            ...
            Log.v(TAG, k21.r("FastJsonParser cost time A ", Long.valueOf(System.currentTimeMillis() - currentTimeMillis)));
            Node parse = parse(parseObject.getJSONObject("data"));
            Log.v(TAG, k21.r("FastJsonParser cost time B ", Long.valueOf(System.currentTimeMillis() - currentTimeMillis)));
            return parse;
        } catch (Exception e) {
            e.printStackTrace();
            throw e;
