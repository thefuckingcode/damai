        ...
        for (BidInfo bidInfo : bitInfoByAdType) {
            if (bidInfo != null && !TextUtils.isEmpty(bidInfo.getCreativeUrl())) {
                if (!TextUtils.isEmpty(bidInfo.getCreativePath())) {
                    str = bidInfo.getCreativePath().trim();
                } else {
                    str = appendUrlParams(bidInfo.getCreativeUrl().trim(), "ykVideoShowType=2");
                    if (Utils.isYoukuOrHuaweiBaipai(this.mContext)) {
                        P2pManager.Result pcdnUrl = P2pManager.getInstance(this.mContext.getApplicationContext()).getPcdnUrl(this.mContext, PcdnType.VOD, str, null);
                ...
                if (TextUtils.isEmpty(str)) {
                    ...
                    period.addSource(new Source(str, (double) bidInfo.getDuration()));
                }
            }
        }
        return period;
