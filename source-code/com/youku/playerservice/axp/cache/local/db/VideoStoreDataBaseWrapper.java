    ...
    public static AbstractStorageVideoStoreDataBase getDatabase(Context context) {
        if (INSTANCE == null) {
            synchronized (AbstractStorageVideoStoreDataBase.class) {
                if (INSTANCE == null) {
                    INSTANCE = (AbstractStorageVideoStoreDataBase) Room.databaseBuilder(context.getApplicationContext(), AbstractStorageVideoStoreDataBase.class, DATABASE_NAME).allowMainThreadQueries().addCallback(new RoomDatabase.Callback() {
                        /* class com.youku.playerservice.axp.cache.local.db.VideoStoreDataBaseWrapper.AnonymousClass1 */
