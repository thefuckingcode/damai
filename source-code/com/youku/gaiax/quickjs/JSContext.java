...
public class JSContext implements TypeAdapter.Context, Closeable {
    ...
    public JSValue wrapAsJSValue(long j) {
        ...
        if (j != 0) {
            try {
                ...
                if (valueTag == -8) {
                    jSValue = new JSSymbol(j, this);
                } else if (valueTag == -7) {
                    jSValue = new JSString(j, this, QuickJS.getValueString(this.pointer, j));
                } else if (valueTag != -1) {
                    if (valueTag == 0) {
                        jSValue = new JSInt(j, this, QuickJS.getValueInt(j));
                    } else if (valueTag == 1) {
                        jSValue = new JSBoolean(j, this, QuickJS.getValueBoolean(j));
                    } else if (valueTag == 2) {
                        jSValue = new JSNull(j, this);
                    } else if (valueTag == 3) {
                        jSValue = new JSUndefined(j, this);
                    } else if (valueTag == 6) {
                        QuickJS.destroyValue(this.pointer, j);
                        throw new JSEvaluationException(QuickJS.getException(this.pointer));
                    } else if (valueTag != 7) {
                        jSValue = new JSInternal(j, this);
                    } else {
                        jSValue = new JSFloat64(j, this, QuickJS.getValueFloat64(j));
                    }
                } else if (QuickJS.isValueFunction(this.pointer, j)) {
                    jSValue = new JSFunction(j, this);
                } else if (QuickJS.isValueArray(this.pointer, j)) {
                    jSValue = new JSArray(j, this);
                } else if (QuickJS.isValueArrayBuffer(this.pointer, j)) {
                    jSValue = new JSArrayBuffer(j, this);
                } else {
                    jSValue = new JSObject(j, this, QuickJS.getValueJavaObject(this.pointer, j));
                }
                this.cleaner.register(jSValue, j);
                return jSValue;
            } catch (Exception e) {
    ...
    public JSNumber createJSNumber(long j) {
        ...
        synchronized (this.jsRuntime) {
            checkClosed();
            jSNumber = (JSNumber) wrapAsJSValue(QuickJS.createValueFloat64(this.pointer, (double) j)).cast(JSNumber.class);
        }
        return jSNumber;
    }
