...
public class TinyWindowConfig {
    ...
    private TINYWINDOW_TYPE mType = TINYWINDOW_TYPE.SYSTEMPIP;
    ...
    private void initFloatingWindowSize(int i, int i2) {
        if ((((float) i) * 1.0f) / ((float) i2) > 1.0f) {
            int round = Math.round((float) (TinyWindowUtils.getScreenWidth(this.mActivity) / 2));
            this.mPlayerViewWidth = round;
            if (round % 2 == 1) {
                this.mPlayerViewWidth = round - 1;
            }
            int round2 = Math.round((float) ((this.mPlayerViewWidth * 9) / 16));
        ...
        int round3 = Math.round((float) ((TinyWindowUtils.getScreenWidth(this.mActivity) * 2) / 5));
        this.mPlayerViewWidth = round3;
        if (round3 % 2 == 1) {
            this.mPlayerViewWidth = round3 - 1;
        }
        int round4 = Math.round((float) ((this.mPlayerViewWidth * 16) / 9));
