    ...
    public static void init(Context context) {
        if (sContext == null) {
            ...
            sContext.registerReceiver(new BroadcastReceiver() {
                /* class com.youku.uplayer.bridging.SystemDelegate.AnonymousClass1 */

                public void onReceive(Context context, Intent intent) {
                    if (ConnectivityBroadcastReceiver.CONNECTIVITY_ACTION.equals(intent.getAction())) {
                        SystemDelegate.sCurrentNetworkType = SystemDelegate.getCurrentNetworkType(context);
                        SystemDelegate.sWifiSSID = SystemDelegate.getWifiSSID(context);
                    }
                }
            }, intentFilter);
        ...
        if (iPlayerBridge != null) {
            ...
            for (int i = bridgeParam.getInt(); i > 0; i--) {
                ...
                if (type == 1) {
                    ...
                    bridgeParam.putDouble((double) sharedPreferences.getFloat(string2, (float) bridgeParam.getDouble()));
                } else if (type == 6) {
