...
public class MultiGridViewUtil {
    ...
    public static double[] getWHFor16bi9(Context context) {
        ...
        if (context.getResources().getConfiguration().orientation == 1) {
            dArr[0] = (double) i;
            dArr[1] = (double) ((i * 9) / 16);
        } else {
            dArr[0] = (double) ((i2 * 16) / 9);
            dArr[1] = (double) i2;
        }
        return dArr;
    }
    ...
    public static int px2dip(Context context, float f) {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "-726097503")) {
            return (int) ((f / context.getResources().getDisplayMetrics().density) + 0.5f);
        }
        return ((Integer) ipChange.ipc$dispatch("-726097503", new Object[]{context, Float.valueOf(f)})).intValue();
    }
}
