...
public class LoadingImageView extends AppCompatImageView {
    ...
    private void createShader() {
        ...
        for (int i = 0; i < width; i++) {
            float sin = (float) (((double) this.mDefaultWaterLevel) + (((double) this.mDefaultAmplitude) * Math.sin(((double) i) * this.mDefaultAngularFrequency)));
            float f = (float) i;
            canvas.drawLine(f, sin, f, (float) height, paint);
            fArr[i] = sin;
        }
        ...
        for (int i3 = 0; i3 < width; i3++) {
            float f2 = (float) i3;
            canvas.drawLine(f2, fArr[(i3 + i2) % width], f2, (float) height, paint);
        }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (this.mWaveShader != null) {
            ...
            this.mShaderMatrix.postRotate((float) rotation, ((float) getWidth()) / 2.0f, ((float) getHeight()) / 2.0f);
            this.mWaveShader.setLocalMatrix(this.mShaderMatrix);
            this.bitmapCanvas.drawRect(0.0f, 0.0f, ((float) getWidth()) - 0.0f, ((float) getHeight()) - 0.0f, this.mViewPaint);
        } else {
