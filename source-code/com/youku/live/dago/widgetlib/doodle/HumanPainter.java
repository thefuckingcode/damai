...
public class HumanPainter extends FrameLayout {
    ...
    public boolean onInterceptTouchEvent(MotionEvent motionEvent) {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "-1409381667")) {
            return isEnabled();
        }
        return ((Boolean) ipChange.ipc$dispatch("-1409381667", new Object[]{this, motionEvent})).booleanValue();
    }
    ...
    public boolean onTouchEvent(MotionEvent motionEvent) {
        ...
        if (AndroidInstantRuntime.support(ipChange, "-555575897")) {
            ...
            if (action == 0) {
                ...
                float sqrt = (float) (((double) this.mCurrentDrawDistance) + Math.sqrt((double) ((f * f) + (f2 * f2))));
                this.mCurrentDrawDistance = sqrt;
                if (sqrt >= ((float) this.mDrawingDistance)) {
                    draw(x, y);
                    this.mCurrentDrawDistance = 0.0f;
                }
            }
