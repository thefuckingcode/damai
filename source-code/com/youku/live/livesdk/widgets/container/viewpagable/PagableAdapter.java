...
public class PagableAdapter extends PagerAdapter implements ViewPager.OnPageChangeListener {
    ...
    public LiveContentView getContentViewByIndex(int i) {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "-830606554")) {
            return this.mRoomListInUse.get(Integer.valueOf(i));
        }
        return (LiveContentView) ipChange.ipc$dispatch("-830606554", new Object[]{this, Integer.valueOf(i)});
    }
    ...
    public int getCount() {
        ...
        Log.e("LivePagableAdapter", "getCount() = " + i);
        return i;
    }
    ...
    public void onPageScrolled(int i, float f, int i2) {
        ...
        if (i4 != i5) {
            ...
            if (this.mLastSelected == i4) {
                getContentViewByIndex(i5).getInstance().didDisappear();
                getContentViewByIndex(i4).getInstance().didAppear();
            } else {
                getContentViewByIndex(i4).getInstance().didAppear();
            }
            Log.e("LivePagableAppear", "didAppear('" + i4 + "')");
        }
        Log.e("LivePagableAdapter", "onPageScrolled('" + i + "', '" + i4 + "', '" + f + "', '" + i2 + "')");
    }
