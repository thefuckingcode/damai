...
public class DanmakuData implements IDanmakuData {
    ...
    public float getBorderSize() {
        ...
        } else if (!checkValue("borderSize")) {
            return 0.0f;
        } else {
            try {
                return (float) ContextUtils.dip2px(Arch.getApp(), (int) Double.parseDouble(String.valueOf(this.valueMap.get("borderSize"))));
            } catch (Exception e) {
                e.printStackTrace();
                return 0.0f;
            }
    ...
    public float getFontBorderSize() {
        ...
        } else if (!checkValue("fontBorderSize")) {
            return 0.0f;
        } else {
            try {
                return (float) ContextUtils.dip2px(Arch.getApp(), (int) Float.valueOf(String.valueOf(this.valueMap.get("fontBorderSize"))).floatValue());
            } catch (Exception e) {
                e.printStackTrace();
                return 0.0f;
            }
    ...
    public boolean isFontColorGradient() {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "399349549")) {
            return checkValue("fontColorTop") && checkValue("fontColorBottom");
        }
        return ((Boolean) ipChange.ipc$dispatch("399349549", new Object[]{this})).booleanValue();
    }
    ...
    public boolean isShowImmediately() {
        ...
        } else if (checkValue("immediately")) {
    ...
    public void setFontColorGradient(Paint paint, int i, int i2) {
        ...
        if (AndroidInstantRuntime.support(ipChange, "-1532050438")) {
            ...
            try {
                paint.setShader(new LinearGradient((float) (i / 2), 0.0f, (float) (i / 2), (float) i2, new int[]{Color.parseColor((String) this.valueMap.get("fontColorTop")), Color.parseColor((String) this.valueMap.get("fontColorBottom"))}, (float[]) null, Shader.TileMode.REPEAT));
            } catch (Exception e) {
                e.printStackTrace();
            }
        } else {
