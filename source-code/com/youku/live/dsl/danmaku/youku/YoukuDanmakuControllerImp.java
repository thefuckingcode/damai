...
public class YoukuDanmakuControllerImp implements IDanmakuController {
    ...
    private synchronized void addDanmuImp(IDanmakuData iDanmakuData) {
        ...
        if (AndroidInstantRuntime.support(ipChange, "629032844")) {
        ...
        } else if (iDanmakuData != null && this.mHandler != null) {
            ...
            if (createDanmaku != null && iDanmakuView != null) {
                ...
                if (currentTime - j < ((long) i2)) {
                    createDanmaku.time = j + ((long) i2);
                }
                long j2 = createDanmaku.time;
                this.timeCache = j2;
                if (iDanmakuData.isShowImmediately()) {
                    createDanmaku.setDanmakuLine(createDanmaku.index % 3);
                } else if (this.timeCache - ((long) this.DANMU_DURATION) > this.mDrawHandlerCallback.getTimeShown()) {
                    return;
                }
                this.lastDanmakuTime = j2;
                createDanmaku.textSize = (float) ContextUtils.dip2px(Arch.getApp(), iDanmakuData.getFontSize());
                ...
                this.mHandler.sendMessageDelayed(obtainMessage, (long) this.POST_DELAY_TIME);
            }
        }
    }
    ...
    private synchronized Duration getDuration() {
        ...
        if (this.mDuration == null) {
            this.mDuration = new Duration((long) this.DANMU_DURATION);
        }
