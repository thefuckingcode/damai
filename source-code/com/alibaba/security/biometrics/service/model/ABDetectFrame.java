...
public class ABDetectFrame extends ABFaceFrame {
    ...
    public ABDetectFrame(ABJniDetectResult aBJniDetectResult, byte[] bArr, int i, int i2, int i3) {
        ...
        if (i5 > 1 && i4 > 1) {
            ...
            float f2 = (float) i4;
            rectF.top = ((float) rect.top) / f2;
            rectF.bottom = ((float) rect.bottom) / f2;
        }
        ...
        if (this.detectInfo.getGestureProgress() > 0.0f && ((double) this.detectInfo.getGestureProgress()) <= 0.5d) {
            this.detectInfo.setActionRange(1);
        } else if (((double) this.detectInfo.getGestureProgress()) > 0.5d && this.detectInfo.getGestureProgress() < 1.0f) {
        ...
        if (aBJniDetectResult.detectType() == ABJniDetectType.DETECT_TYPE_YAW || aBJniDetectResult.detectType() == ABJniDetectType.DETECT_TYPE_YAW_STILL) {
            float f3 = aBJniDetectResult.yawScore;
            if (f3 > 0.0f) {
    ...
    public int facesDetected() {
        ...
        if (aBJniDetectResult == null || !aBJniDetectResult.faceExist) {
            return 0;
        }
        return 1;
    }
