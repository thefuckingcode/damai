...
public class FinishState extends BaseState {
    ...
    private void doDetectSuccess() {
        ...
        if (this.mABParams.recapEnable) {
            ABFaceRecapDetector.getInstance().getRecapResult(new ABFaceRecapDetector.OnEventListener() {
                /* class com.alibaba.security.biometrics.service.state.FinishState.AnonymousClass1 */
                ...
                public void onResult(int i, float[] fArr, String str) {
                    ...
                    if (ABFaceRecapDetector.getInstance().getFrameCount() > 0) {
                        ABDetectContext.getInstance().getResult().setRecapAvgTime((int) (ABFaceRecapDetector.getInstance().getDetectTotalTime() / ((long) ABFaceRecapDetector.getInstance().getFrameCount())));
                    }
