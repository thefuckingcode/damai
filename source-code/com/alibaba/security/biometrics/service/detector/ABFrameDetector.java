...
public class ABFrameDetector extends ABDetector {
    ...
    private boolean enableAndInitBioJni(int i, int i2, int i3) {
        ...
        if (!ALBiometricsJni.IsEnabled()) {
            ...
            ALBiometricsParams aLBiometricsParams = this.mParams;
            if (aLBiometricsParams.lessImageMode) {
                ALBiometricsJni.SetParameter(26, (float) aLBiometricsParams.bigImageSize);
            }
            float f = 1.0f;
            ALBiometricsJni.SetParameter(37, this.mParams.detectWrongAction ? 1.0f : 0.0f);
            if (!this.mParams.detectOcclusion) {
                f = 0.0f;
            }
            ALBiometricsJni.SetParameter(38, f);
            int i5 = this.mParams.bgDetectTimeIntervals;
            if (i5 >= 0) {
                ALBiometricsJni.SetParameter(39, (float) i5);
            }
            int i6 = this.mParams.bgDetectColorThreshold;
            if (i6 >= 0) {
                ALBiometricsJni.SetParameter(40, (float) i6);
            }
            this.reflectCmd = -1;
    ...
    private boolean handleResult(byte[] bArr, int i, int i2, int i3, ABJniDetectResult aBJniDetectResult) {
        ...
        this.mDetectEventListener.onFrameDetected(10, aBDetectFrame);
        return true;
    }
    ...
    public boolean init(Context context, ALBiometricsParams aLBiometricsParams) {
        ...
        if (this.mParams == null) {
            this.mParams = (ALBiometricsParams) new ABParamsHelper(new Bundle()).getParams();
        }
        ...
        if (bArr == null && aLBiometricsParams2.licenseTimeData == null) {
            i = ALBiometricsJni.checkLicense(context, this.mALBiometricsEventListener);
        } else {
            i = ALBiometricsJni.checkLicense(context, bArr, aLBiometricsParams2.licenseTimeData);
        }
        boolean z = i == 0;
        this.isLoaded = z;
        if (!z) {
            handleError(GlobalErrorCode.ERROR_ALGO_SO_LOAD_FAIL, String.valueOf(i));
        ...
        if (this.mParams.recapEnable) {
            ABFaceRecapDetector instance = ABFaceRecapDetector.getInstance();
            this.mFaceRecapDetector = instance;
            instance.init(this.mContext, this.mParams, this.mALBiometricsEventListener);
        }
        return this.isLoaded;
    }
