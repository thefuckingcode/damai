        ...
        if (jSONLexer.token == 12) {
            ...
            while (true) {
                try {
                    ...
                    if (c == '\"') {
                        str = jSONLexer.scanSymbol(defaultJSONParser.symbolTable, jl1.QUOTE);
                        jSONLexer.skipWhitespace();
                        if (jSONLexer.ch != ':') {
                    ...
                    if (str != JSON.DEFAULT_TYPE_KEY || jSONLexer.isEnabled(Feature.DisableSpecialKeyDetect)) {
                        ...
                        Class<?> checkAutoType = defaultJSONParser.config.checkAutoType(jSONLexer.scanSymbol(defaultJSONParser.symbolTable, jl1.QUOTE), null, jSONLexer.features);
