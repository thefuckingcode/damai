...
public class YouKuCrashReporter {
    ...
    public static void initYouKuCrashReporter(final Context context) {
        try {
            ...
            if (MotuCrashReporter.getInstance().enable(context, defaultAppkey + "@android", defaultAppkey, contextAppVersion, "channel", null, reporterConfigure)) {
                LogUtil.d("crashreporter enable success");
            } else {
                LogUtil.d("crashreporter enable failure");
            }
            MotuCrashReporter.getInstance().setCrashCaughtListener(new IUTCrashCaughtListener() {
                /* class com.alibaba.motu.crashreporter.YouKuCrashReporter.AnonymousClass1 */
