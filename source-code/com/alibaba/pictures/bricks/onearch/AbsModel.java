...
public abstract class AbsModel<D extends IItem<ItemValue>, VALUE> extends com.alient.onearch.adapter.view.AbsModel<D, VALUE> {
    ...
    public TrackInfo compatParseTrackInfo(D d) {
        String str;
            ...
            if (jSONObject2 != null) {
                ...
                if (!TextUtils.isEmpty(jSONObject2.getString("spmd"))) {
                    if (jSONObject2.getString("spmd").contains(JSMethod.NOT_SET)) {
                        str = jSONObject2.getString("spmd") + d.getIndex();
                    } else {
                        str = jSONObject2.getString("spmd");
                    }
                    trackInfo.setSpmd(str);
                }
    ...
    public TrackInfo getCompatTrackInfo() {
        IpChange ipChange = $ipChange;
        if (!AndroidInstantRuntime.support(ipChange, "1881904846")) {
            return this.compatTrackInfo;
        }
        return (TrackInfo) ipChange.ipc$dispatch("1881904846", new Object[]{this});
    }
